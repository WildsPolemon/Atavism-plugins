    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mb-6 text-sm text-slate-400">Багатоточковий облік (як AinurPOS)</p>
        </div>
      ...
      {show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <form onSubmit={save} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d111c] p-6">
            <input required placeholder="Назва магазину" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mb-3 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-white" />
            <input placeholder="Адреса" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="mb-3 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-white" />
            <input placeholder="Телефон" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="mb-4 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-2 text-white" />
