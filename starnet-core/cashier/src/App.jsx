...
export default function App() {
  ...
  const loadCatalog = useCallback(() => {
    api.products(categoryId || undefined).then((r) => setCatalog(r.products || []));
  }, [categoryId]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === 'Escape') {
        setMenuOpen(false); setPayOpen(false); setCustomerOpen(false); setEditItem(null); setJournalOpen(false);
      }
      if (e.key === 'f' || e.key === 'F') { if (!payOpen && !customerOpen) { e.preventDefault(); searchRef.current?.focus(); } }
      if (e.key === 'c' || e.key === 'C') { if (!payOpen) { e.preventDefault(); setCustomerOpen(true); } }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [payOpen, customerOpen]);
  ...
  const onSearch = async () => {
    ...
    if (/^\d{4,}$/.test(query)) {
      try {
        ...
        if (r.product_id) { addToCart({ id: r.product_id, name: r.name, sale_price: r.retail_price, retail_price: r.retail_price }); setQ(''); return; }
      } catch { /* */ }
    }
  ...
  const completePay = async ({ payment_cash, payment_card, notes, print_receipt }) => {
    await api.sale({
    ...
    <div className="flex h-screen flex-col bg-ainur-bg">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <CartPanel
          ...
          subtotal={subtotal}
          discountPct={custDiscPct}
          discountAmt={custDiscAmt}
          total={total}
          onRemove={(id) => setCart((c) => c.filter((x) => x.product_id !== id))}
          onEdit={setEditItem}
          onSale={() => shift ? setPayOpen(true) : setErr('Відкрийте зміну')}
          onAddSuggestion={addToCart}
        />
      </div>

      <FooterBar
        storeName={settings.company_name || 'StarNet Core'}
        shift={shift}
        total={total}
        onMenu={() => setMenuOpen(true)}
        onSale={() => shift && cart.length ? setPayOpen(true) : setErr('Відкрийте зміну або додайте товари')}
        cartEmpty={!cart.length}
      />

      {menuOpen && <SideMenu user={user} onClose={() => setMenuOpen(false)} onAction={menuAction} />}
      {payOpen && <PaymentScreen total={total} customer={customer} onClose={() => setPayOpen(false)} onPay={completePay} />}
      ...
      {journalOpen && (
        <ReceiptJournal
          ...
          onReturn={async (id) => { await api.returnSale(id); setSales((await api.sales()).sales || []); }}
        />
      )}
