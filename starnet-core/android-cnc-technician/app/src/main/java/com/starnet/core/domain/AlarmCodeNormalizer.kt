...
object AlarmCodeNormalizer {
    fun normalize(controller: String, raw: String, fanucTypeHint: String? = null): String {
        ...
        val prefixed = Regex("""^(PS|SV|SP|OT|OH|DS|PW|SW|SR|BG|APC)([0-9]{1,4})$""").find(token)
        if (prefixed != null) {
        ...
        val legacyPs = Regex("""^P/S([0-9]{1,4})$""").find(token)
        if (legacyPs != null) return "PS${legacyPs.groupValues[1].padStart(4, '0')}"

        val plain = Regex("""^([0-9]{1,4})$""").find(token) ?: return token
