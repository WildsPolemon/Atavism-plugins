...
//					}
			...
			for (CombatBuildingTarget targetInfo : buildingTargets) {
				...
				if (Log.loggingDebug)
					Log.debug("AttackAbility.runTargetActivation: targetInfo = " + targetInfo);
                Map<String, Serializable>params = new HashMap<>();
				params.put("skillType", skillType);
				params.put("result", attackerResult);
				params.put("hitRoll", hitRoll);
...
//					}
		...
		if(effectApplyed){
			...
			if(info.isUser() && target.isUser()) {
				if (!target.getOid().equals(info.getOwnerOid())) {
					if (target.getPvpMark() == 1) {
						int mark = info.getPvpMark();
						info.setPvpMark(mark <= 1 ? 1 : 2);
						target.setPvpMark(1);
					}
				}
				int pvpMode = (int) info.getProperty(FactionStateInfo.PVP_MODE_PROP);
				if (pvpMode == 2 && info.getPvpMark() <= 1) {
					info.setPvpMark(1);
				}
				for (CombatInfo targetInfo : targets) {
