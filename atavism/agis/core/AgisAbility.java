...
public class AgisAbility {
    ...
    public  void setPvpEnterExit(boolean pvp) { pvp_enter_exit = pvp; }
    public boolean isPvpEnterExit(){
        return pvp_enter_exit;
    }

    protected boolean pvp_enter_exit = false;
...
//    }
    ...
    public ArrayList<CombatInfo> getAoETargets(CombatInfo caster, CombatInfo targetInfo, Point loc , float minRange, float maxRange, AgisAbilityState state ) {
        ...
        if (Log.loggingDebug)Log.debug("Ability.getAoETargets id "+getID()+" caster"+caster.getOid()+" CasterYaw="+casterNode);
       ...
       // DebugAbility(casterNode, caster);
		...
		if (targetType == TargetType.AREA && (targetSubType == TargetSubType.ENEMY || targetSubType == TargetSubType.FRIEND_OR_ENEMY)) {
            ...
            if(caster.getBooleanProperty("pvpAttack") || caster.getPvpMark() > 0)
                attackableTargets.addAll(caster.getPvpChaoticTargets());
            Set<OID> chaot = new HashSet<OID>(caster.getPvpChaoticTargets());
            for (OID t : chaot) {
                CombatInfo tt = CombatPlugin.getCombatInfo(t);
                if(tt.getPvpMark()>0 && !attackableTargets.contains(t)){
                    attackableTargets.add(t);
                }
            }
...
//                        " testLoc1=" + testLoc1 +
    ...
    public void DebugAbilityAngle( CombatInfo caster, CombatInfo target){
        if(CombatPlugin.ABILITY_DEBUG_PLAYER!=null) {
            BasicWorldNode casterNode = WorldManagerClient.getWorldNode(caster.getOwnerOid());
            if(casterNode!=null) {
            Point casterLoc = casterNode.getLoc();
             Quaternion casterQuat = casterNode.getOrientation();


            Quaternion casterQuaternion = new Quaternion(casterQuat);
            Quaternion casterQuaternion60 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() + 60F , casterQuat.getRoll());
            Quaternion casterQuaternion300 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() - 60F , casterQuat.getRoll());

            Quaternion casterQuaternion45 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() + 45F , casterQuat.getRoll());
            Quaternion casterQuaternion315 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() - 45F , casterQuat.getRoll());

            Quaternion casterQuaternion135 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() + 135F, casterQuat.getRoll());
            Quaternion casterQuaternion225 = new Quaternion().setEulerAngles(casterQuat.getPitch(), casterQuat.getYaw() - 135F, casterQuat.getRoll());

                AOVector rayFront = new AOVector(casterLoc);
                rayFront.add(casterQuaternion.getZAxis().multiply(maxRange));
                AOVector ray45 = new AOVector(casterLoc);
                ray45.add(casterQuaternion45.getZAxis().multiply(maxRange));
                AOVector ray135 = new AOVector(casterLoc);
                ray135.add(casterQuaternion135.getZAxis().multiply(maxRange));
                AOVector ray225 = new AOVector(casterLoc);
                ray225.add(casterQuaternion225.getZAxis().multiply(maxRange));
                AOVector ray315 = new AOVector(casterLoc);
                ray315.add(casterQuaternion315.getZAxis().multiply(maxRange));
                AOVector ray60 = new AOVector(casterLoc);
                ray60.add(casterQuaternion60.getZAxis().multiply(maxRange));
                AOVector ray300 = new AOVector(casterLoc);
                ray300.add(casterQuaternion300.getZAxis().multiply(maxRange));



            Map<String, Serializable> props = new HashMap<String, Serializable>();
            props.put("ext_msg_subtype", "ability_targets_angle");
            props.put("targetType", targetType.toString());
            props.put("targetSubType", targetSubType.toString());
            props.put("minRange", minRange);
            props.put("maxRange", maxRange);
            ...
            props.put("orientationz", casterQuaternion.getZ());
            props.put("orientationw", casterQuaternion.getW());
            props.put("locX", casterLoc.getX());
            props.put("locY", casterLoc.getY());
            props.put("locZ", casterLoc.getZ());
            //Front rey
            props.put("locXRF", rayFront.getX());
            props.put("locYRF", rayFront.getY());
            props.put("locZRF", rayFront.getZ());

                props.put("locXR45", ray45.getX());
                props.put("locYR45", ray45.getY());
                props.put("locZR45", ray45.getZ());

                props.put("locXR135", ray135.getX());
                props.put("locYR135", ray135.getY());
                props.put("locZR135", ray135.getZ());

                props.put("locXR225", ray225.getX());
                props.put("locYR225", ray225.getY());
                props.put("locZR225", ray225.getZ());

                props.put("locXR315", ray315.getX());
                props.put("locYR315", ray315.getY());
                props.put("locZR315", ray315.getZ());

                props.put("locXR60", ray60.getX());
                props.put("locYR60", ray60.getY());
                props.put("locZR60", ray60.getZ());

                props.put("locXR300", ray300.getX());
                props.put("locYR300", ray300.getY());
                props.put("locZR300", ray300.getZ());

                BasicWorldNode targetNode = WorldManagerClient.getWorldNode(target.getOwnerOid());
                if(targetNode != null) {
                    props.put("locXT", targetNode.getLoc().getX());
                    props.put("locYT", targetNode.getLoc().getY());
                    props.put("locZT", targetNode.getLoc().getZ());

                    AOVector rayTFront = new AOVector(targetNode.getLoc());
                    rayTFront.add(targetNode.getOrientation().getZAxis().multiply(maxRange));
                    props.put("locXTF", rayTFront.getX());
                    props.put("locYTF", rayTFront.getY());
                    props.put("locZTF", rayTFront.getZ());
                }
                TargetedExtensionMessage extmsg = new TargetedExtensionMessage(WorldManagerClient.MSG_TYPE_EXTENSION, CombatPlugin.ABILITY_DEBUG_PLAYER, CombatPlugin.ABILITY_DEBUG_PLAYER, false, props);
            Engine.getAgent().sendBroadcast(extmsg);
            if(Log.loggingDebug)
                Log.debug("AgisAbility.DebugAbilityAngle " + caster.getOid() + " Loc=" + casterLoc + " casterQuaternion=" + casterQuaternion + " Y=" + casterQuaternion.getYAxis() + " X=" + casterQuaternion.getXAxis() + " Z=" + casterQuaternion.getZAxis());
        }
        }
    }
...
//                    Quaternion casterQuaternion = new Quaternion(casterNode.getOrientation());
	...
	public void beginActivation(AgisAbilityState state) {
		...
		if (getUseEnterCombatState()) {
            combatInfo.setCombatState(true);
            CombatInfo targetInfo = state.getTarget();

                    if(targetInfo != null) {
                        if(!combatInfo.isMob() && !targetInfo.isMob()) {
                         if(!state.getSourceOid().equals(state.getTargetOid())) {

                            int pvpMode = (int) combatInfo.getIntProperty(FactionStateInfo.PVP_MODE_PROP);
                            int lpvpMode = (int) targetInfo.getIntProperty(FactionStateInfo.PVP_MODE_PROP);

                            if(pvpMode==2 && lpvpMode==2) {
	 ...
	 public void completeActivationTargetConsume(AgisAbilityState state) {
		 ...
		 if (aoeType == AoeType.PLAYER_RADIUS || aoeType == AoeType.NONE || aoeType == AoeType.TARGET_RADIUS || aoeType == AoeType.LOCATION_RADIUS) {
				for (CombatInfo ci : targets) {
                    ...
                    if (targetEffectConsumed)
                    {
                        ArrayList<EffectState> effectsToRemove = new ArrayList<EffectState>();

                        for (EffectState existingState : ci.getCurrentEffects())
                            for (int effectTag: existingState.getEffect().getTags())
                                if (effect_tags_on_target.contains(effectTag))
                                {
                                    if (Log.loggingDebug)
                                        Log.debug("ABILITY: AgisAbility.completeActivationTargetConsume(AgisAbilityState): target effect with ID = " + existingState.getEffect().getID() + " being consumed");
                                    effectsToRemove.add(existingState);
                                    break;
                                }
    ...
    public boolean completeActivationCasterConsume(AgisAbilityState state) {
        ...
        if (casterEffectConsumed)
        {
            ArrayList<EffectState> effectsToRemove = new ArrayList<EffectState>();

            for (EffectState existingState : state.getSource().getCurrentEffects())
                for (int effectTag: existingState.getEffect().getTags())
                    if (effect_tags_on_caster.contains(effectTag))
                    {
                        if (Log.loggingDebug)
                            Log.debug("ABILITY: AgisAbility.completeActivationCasterConsume(AgisAbilityState): caster effect with ID = " + existingState.getEffect().getID() + " being consumed");
                        effectsToRemove.add(existingState);
                        break;
                    }
        ...
        if (targetEffectConsumed)
        {
            ...
            for (EffectState existingState : state.getTarget().getCurrentEffects())
                for (int effectTag: existingState.getEffect().getTags())
                    ...
                    {
                        if (Log.loggingDebug)
                            Log.debug("ABILITY: AgisAbility.completeActivationCasterConsume(AgisAbilityState): target effect with ID = " + existingState.getEffect().getID() + " being consumed");
                        effectsToRemove.add(existingState);
                        break;
                    }
 ...
 public void pulseTarget(AgisAbilityState state) {
    ...
    public void pulseConsumeTarget(AgisAbilityState state) {
		if (aoeType == AoeType.PLAYER_RADIUS || aoeType == AoeType.NONE || aoeType == AoeType.TARGET_RADIUS || aoeType == AoeType.LOCATION_RADIUS) {
			for (CombatInfo ci : targets) {
                ...
                if (pulseTargetEffectConsumed)
                {
                    ArrayList<EffectState> effectsToRemove = new ArrayList<EffectState>();

                    for (EffectState existingState : ci.getCurrentEffects())
                        for (int effectTag: existingState.getEffect().getTags())
                            if (pulse_effect_tags_on_target.contains(effectTag))
                            {
                                if (Log.loggingDebug)
                                    Log.debug("ABILITY: AgisAbility.pulseConsumeTarget(AgisAbilityState): target effect with ID = " + existingState.getEffect().getID() + " being consumed");
                                effectsToRemove.add(existingState);
                                break;
                            }
	...
	//if (!consumeOnActivation) {
       ...
       // sada
        ...
        if (pulseCasterEffectConsumed)
        {
            ArrayList<EffectState> effectsToRemove = new ArrayList<EffectState>();

            for (EffectState existingState : state.getSource().getCurrentEffects())
                for (int effectTag: existingState.getEffect().getTags())
                    if (pulse_effect_tags_on_caster.contains(effectTag))
                    {
                        if (Log.loggingDebug)
                            Log.debug("ABILITY: AgisAbility.completeActivationCasterConsume(AgisAbilityState): caster effect with ID = " + existingState.getEffect().getID() + " being consumed");
                        effectsToRemove.add(existingState);
        ...
        if (Log.loggingDebug)
            Log.debug( "AgisAbility.checkTarget: obj=" + caster + " isUser=" + caster.isUser() + " target="
		       + target + " attackable=" + ((target==null)?"N/A":target.attackable())
                    ...
                    +" Caster pvpMode"+caster.getProperty(FactionStateInfo.PVP_MODE_PROP)
                    +" target pvpMode"+((target==null)?"N/A":target.getProperty(FactionStateInfo.PVP_MODE_PROP))
            );
        	...
        	case AREA:
                if(targetSubType == TargetSubType.ENEMY || targetSubType == TargetSubType.FRIEND_OR_ENEMY) {
                    if (aoeType == AoeType.TARGET_RADIUS) {
                        ...
                        if ((caster.getAttackableTargets().containsKey(target.getOwnerOid())) || (caster.getPvpChaoticTargets().contains(target.getOwnerOid()) && (caster.getBooleanProperty("pvpAttack") || caster.getPvpMark() > 0))) {
                            return AbilityResult.SUCCESS;
                        } else {
                            return AbilityResult.INVALID_TARGET;
                        }
                    ...
                    return AbilityResult.SUCCESS;
                        ...
                        if (aoeType == AoeType.TARGET_RADIUS) {
                            if (target.equals(caster) || caster.getFriendlyTargets().containsKey(target.getOwnerOid()) && !caster.getPvpChaoticTargets().contains(target.getOwnerOid())) {
                                return AbilityResult.SUCCESS;
                            } else {
                                return AbilityResult.INVALID_TARGET;
                            }
            ...
            case SINGLE_TARGET:
                if(targetSubType == TargetSubType.FRIEND_NOT_SELF) {
                        ...
                        if (target.equals(caster))
                            return AbilityResult.INVALID_TARGET;
                        if (caster.getFriendlyTargets().containsKey(target.getOwnerOid()) && !caster.getPvpChaoticTargets().contains(target.getOwnerOid())) {
                            return AbilityResult.SUCCESS;
                        } else {
                            return AbilityResult.INVALID_TARGET;
                        }
                    ...
                    if (!target.attackable() && targetSubType == TargetSubType.ENEMY) {
                        if (reqTarget) {
                    ...
                    if(targetSubType == TargetSubType.FRIEND_OR_ENEMY){
                        if(caster.getAttackableTargets().containsKey(target.getOwnerOid()) || caster.getFriendlyTargets().containsKey(target.getOwnerOid())){
                            return AbilityResult.SUCCESS;
                        }
                    }

                    if (Log.loggingDebug)
                        Log.debug("AgisAbility.TargetCheck caster.getAttackableTargets" + caster.getAttackableTargets());

                    if ((caster.getAttackableTargets().containsKey(target.getOwnerOid())) || (caster.getPvpChaoticTargets().contains(target.getOwnerOid()) && ((caster.getBooleanProperty("pvpAttack") || caster.getPvpMark() > 0)|| target.getPvpMark() > 0 ))) {
                        Log.debug("AgisAbility.TargetCheck not attackable and not reqTarget");
                        return AbilityResult.SUCCESS;
                    } else {
...
//    	    }
     ...
     */
    protected AbilityResult checkPosition(CombatInfo obj, CombatInfo target, ActivationState state, AgisAbilityState abilityState, BasicWorldNode attackerNode) {
    	if(Log.loggingDebug)Log.debug("AgisAbility.checkPosition: reqFacingTarget="+reqFacingTarget+" obj.isUser="+obj.isUser());
    	if (aoeType == AoeType.LOCATION_RADIUS) {
			return AbilityResult.SUCCESS;
		}

        DebugAbilityAngle(obj, target);
    	...
    	/*	if (playerYaw < 0)
    		...
    		if (motionYaw < 0)
    			motionYaw += 360;*/
    		if(Log.loggingDebug)Log.debug("ANGLE: obj="+obj+" player orient: " + playerYaw + " and motionYaw = " + motionYaw+" yaw= "+yaw);
    		if (Math.abs(yaw) > 60) {
    			if (reqTarget) {
                    ...
                    if (petCountStat > 0) {
                        double petTypeCountLimit = objToCheck.statGetCurrentValueWithPrecision(CombatPlugin.STAT_NAMES.get(petCountStat));
                            ...
                            if (!AgisMobPlugin.PET_ALLOW_TYPE_LIMIT_EXCEEDED && petTypeCountLimit <= petProfiles.get(pId)) {
                                if(Log.loggingDebug)  Log.debug("PET: type " + petCountStat + ":" + CombatPlugin.STAT_NAMES.get(petCountStat) + " limit reached ");
    ...
    protected AbilityResult checkPvpMode(CombatInfo obj ){
        if(isPvpEnterExit()) {
            int pvpMode = -1;
                if(obj.getProperty(FactionStateInfo.PVP_MODE_PROP)!=null) {
                    pvpMode = (int) obj.getProperty(FactionStateInfo.PVP_MODE_PROP);
                }
               if(pvpMode == 1){
                   return AbilityResult.SUCCESS;
               }else{
                   return AbilityResult.NO_PVP;
               }
        }else{
            return AbilityResult.SUCCESS;
        }
    ...
    protected AbilityResult checkAbility(CombatInfo obj, CombatInfo target, ActivationState state, AgisAbilityState abilityState, boolean skipCheckCooldown) {
        ...
        tChecksMid2 = System.nanoTime();
     	...
     	//if(abilityState.getClaimID() > 0)
     		result = checkPosition(obj, target, state,abilityState, casterWNode);
...
//        boolean result = state.updateState();
	...
	public static void deactivateAbility(AgisAbilityState state) {
        Log.debug("deactivateAbility "+state.ability.getID());
		...
		try {
			if (state.getAbility().isToggle()) {
                ...
                if(state.getAbility().isPvpEnterExit()){
                    HashMap<String, Serializable> props = new HashMap<String, Serializable>();
                    props.put("pvpState", false);
                    WorldManagerClient.ExtensionMessage eMsg = new WorldManagerClient.ExtensionMessage(FactionClient.MSG_TYPE_UPDATE_PVP_STATE, state.getSourceOid(), props);
                    Engine.getAgent().sendBroadcast(eMsg);
                }
                List<EffectState> list = state.getEffectStates();
                Log.debug("deactivateAbility list="+list);

                if(list!=null){
                    for (EffectState c : list) {
                        AgisEffect.removeEffect(c);
                    }
                }
...
//    }
    ...
    private boolean casterEffectConsumed = false;
    private boolean targetEffectConsumed = false;
    private boolean pulseCasterEffectConsumed = false;
    private boolean pulseTargetEffectConsumed = false;

    public void setCasterEffectConsumed(boolean value) {  casterEffectConsumed = value;  }
    public void setTargetEffectConsumed(boolean value) {  targetEffectConsumed = value;  }
    public void setPulseCasterEffectConsumed(boolean value) {  pulseCasterEffectConsumed = value;  }
    public void setPulseTargetEffectConsumed(boolean value) {  pulseTargetEffectConsumed = value;  }
    public boolean getCasterEffectConsumed() {  return casterEffectConsumed;  }
    public boolean getTargetEffectConsumed() {  return targetEffectConsumed;  }
    public boolean getPulseCasterEffectConsumed() {  return pulseCasterEffectConsumed;  }
    public boolean getPulseTargetEffectConsumed() {  return pulseTargetEffectConsumed;  }
