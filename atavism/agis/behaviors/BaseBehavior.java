...
public class BaseBehavior extends Behavior implements Runnable {
	...
	public void handleMessage(Message msg, int flags) {
		try {
			...
			if (msg.getMsgType() == Behavior.MSG_TYPE_COMMAND) {
				...
				// task.cancel(true);
				if (Log.loggingDebug)
					Log.debug("BaseBehavior.onMessage: command = " + command + "; oid = " + obj.getOid() + "; name " + obj.getName()+" mdgId "+msg.getMsgId());
				if (command.equals(MSG_CMD_TYPE_GOTO)) {
					...
					if (destination == null) {
						if (Log.loggingDebug) Log.debug("BaseBehavior: goto destination is null");
						return;
					}
					mode = MSG_CMD_TYPE_GOTO;
					roamingBehavior = true;
					gotoSetup(destination, gotoMsg.getSpeed());
					io.micrometer.core.instrument.Timer.builder("handle_goto").tag("handle","base_behavior")
							.register(Prometheus.registry()).record(Duration.ofNanos(System.nanoTime() - tStart));
				} else if (command.equals(MSG_CMD_TYPE_GOTO_ROAM)) {
					if(System.currentTimeMillis() - lastRoamTime < 100L){
						if (Log.loggingDebug)	Log.debug("BaseBehavior: Roam Skip "+(System.currentTimeMillis() - lastRoamTime));
						return;
					}
					long tStart = System.nanoTime();
					if (task != null)
					...
					gotoSetup(destination, gotoMsg.getSpeed());
					io.micrometer.core.instrument.Timer.builder("handle_roam").tag("handle","base_behavior")
							.register(Prometheus.registry()).record(Duration.ofNanos(System.nanoTime() - tStart));
	...
	public void gotoSetup(Point dest, float speed) {
		...
		if (Log.loggingDebug)Log.debug("BaseBehavior.gotoSetup Start oid=" + obj.getOid());
		...
		if (myLoc != null && dest != null) {
			...
			if (Log.loggingDebug)Log.debug("BaseBehavior.gotoSetup End oid=" + obj.getOid() );
		}
	}
	...
	public void gotoUpdate() {
		...
		io.micrometer.core.instrument.Timer.builder("handle_roam").tag("handle","base_goto_update")
				.register(Prometheus.registry()).record(Duration.ofNanos(System.nanoTime() - tStart));
	}
...
//		float dist = Point.distanceTo(destLoc, myLoc);
	...
	protected boolean interpolatePath() {
		...
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.interpolatePath: oid=" + oid + " || locAndDir=" + locAndDir + " nomove=" + nomove + " timeNow=" + timeNow+" pathState startLoc="+pathState.getStartLoc()+" endLoc="+pathState.getEndLoc());

		if (locAndDir == null) {
			Log.debug("BaseBehavior.interpolatePath: locAndDir is null");
			Point loc = pathState.getEndLoc()==null?obj.getWorldNode().getLoc():pathState.getEndLoc();
			obj.getWorldNode().setDir(new AOVector(0, 0, 0));
			obj.getWorldNode().setPathInterpolatorValues(timeNow, new AOVector(0, 0, 0), loc, Quaternion.Identity);
			...
			if (interpolatingPath) {
				...
				cancelPathInterpolator(oid, loc);
				interpolatingPath = false;
			}
			if (Log.loggingDebug)
				Log.debug("BaseBehavior.interpolatePath: loc=" + loc);
			// obj.updateWorldNode();
		} else {
