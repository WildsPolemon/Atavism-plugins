	...
	//	long start = System.nanoTime();
		...
		try {
			...
			if (msg.getMsgType() == AgisMobClient.MSG_TYPE_PET_COMMAND_UPDATE) {
				...
				} else if (propMsg.getPropertyMapRef().containsKey(FactionStateInfo.PET_FACTION_PROP)) {
					if (propMsg.getSubject().equals(ownerOid)) {
						String tempFaction = (String) propMsg.getProperty(FactionStateInfo.PET_FACTION_PROP);
						...
						EnginePlugin.setObjectProperty(obj.getOid(), Namespace.FACTION, FactionStateInfo.PET_FACTION_PROP, tempFaction);
					} else {
