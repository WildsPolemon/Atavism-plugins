            ...
            if (event.equals(BaseBehavior.MSG_EVENT_TYPE_ARRIVED)) {
                if (!inCombat) {
					...
					if (scheduledCheck != null && !scheduledCheck.isDone()) {
						long delay = scheduledCheck.getDelay(TimeUnit.MILLISECONDS);
						if(delay > lingerTime) {
...
//					Log.error("RadiusRoamBehavior: "+obj.getOid()+" schedule "+lingerTime);
					scheduledCheck =  Engine.getExecutor().schedule(this, lingerTime, TimeUnit.MILLISECONDS);
                }
            }
...
//					Log.dumpStack("RadiusRoamBehavior: "+obj.getOid()+" schedule "+lingerTime);
					scheduledCheck = Engine.getExecutor().schedule(this, lingerTime, TimeUnit.MILLISECONDS);
                }
            }
        }
    ...
    public void startRoam() {
		scheduledCheck= Engine.getExecutor().schedule(this, 1000L, TimeUnit.MILLISECONDS);
    }
