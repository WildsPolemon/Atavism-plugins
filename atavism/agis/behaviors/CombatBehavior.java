    ...
    public void handleMessage(Message msg, int flags) {
        ...
        */
			...
			if (event.equals(BaseBehavior.MSG_EVENT_TYPE_ARRIVED)) {
				...
				handleArrived();
                io.micrometer.core.instrument.Timer.builder("cb_handle_arrived")//.tag("section", "death")
...
protected void attackTarget(OID targetOid) {
	...
	boolean runAbility(int id) {
		...
		if (((minrange<=0 && distance>=0 )|| distance  >= minrange + targetHitBoxRange + hitBoxRange )&& distance  <= maxrange + targetHitBoxRange + hitBoxRange) {
...
//                        }
				...
				if (distance < min || distance > max || System.currentTimeMillis() - lastUseAbility <  selectedBehavior.abilityinterval) {
					...
					if(speed > 0) {
                        ...
                        goreturn = false;
                        if (Log.loggingDebug)
                            Log.debug("CombatBehavior.runAbility: Ranged defensive  move to position " + _followLoc);
                        Point newPoint = AgisMobPlugin.findNearestPoint(obj.getOid(), _followLoc, obj.getInstanceOid());
...
private void checkDistance() {
    ...
    void threatUpdatedInternal() {
        ...
        if (CombatPlugin.PERCEPTION_STEALTH_STAT != null) {
            ...
            if (props != null && props.containsKey(CombatPlugin.PERCEPTION_STEALTH_STAT) && props.get(CombatPlugin.PERCEPTION_STEALTH_STAT) != null) {
        ...
        if (CombatPlugin.STEALTH_STAT != null && currentTarget != null) {
            ...
            if (props != null && props.containsKey(CombatPlugin.STEALTH_STAT) && props.get(CombatPlugin.STEALTH_STAT) != null) {
...
//            }
	...
	public class BehaviorLogicTask implements Runnable {
		...
		public void run() {
            ...
            io.micrometer.core.instrument.Timer.builder("cb_behavior_logic")
                    .register(Prometheus.registry()).record(Duration.ofNanos(System.nanoTime() - tStart));
			if (Log.loggingDebug)
	...
	void BehaviorLogic() {
		...
		if(selectedBehavior!=null) {
            CombatBehaviorEntry selectedBehavior = this.selectedBehavior;
