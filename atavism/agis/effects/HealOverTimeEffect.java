      ...
      //  effectSkillType = params.get("skillType");
    	...
    	for (EffectState existingState : effset) {
    		if (existingState.getEffect().getID() == getID() && !state.equals(existingState)) {
    			...
    			healFromEffects +=ef.getPulseHeal(existingState);
    			hasThisEffect = true;
    			similarEffect = existingState;
    		    if (source.getOwnerOid().equals(similarEffect.getStackCaster())) {
    		    	healFromEffectsSame +=ef.getPulseHeal(similarEffect);
    		    	fromThisCaster = true;
    	...
    	if (stackCase == 1) {
    		...
    		//	AgisEffect.removeEffect(sameCasterEffect);
				...
				sameCasterEffect.getParams().put("pulseHeal", pulseHeal * stackLevel);
...
//				se.setPulseHeal(pulseHeal * stackLevel);
		...
		params.put("pulseHeal", pulseHeal );
	...
	public void pulse(EffectState state) {
		...
		int pulseHeal =  (int)params.getOrDefault("pulseHeal",0);
    ...
    public int getPulseHeal(EffectState state) {
		if (state != null) {
			return (int) state.getParams().getOrDefault("pulseHeal", 0);
		}
		return 0;

	}
    public void setPulseHeal(EffectState state, int hps) {
		if (state != null) {
			state.getParams().put("pulseHeal", hps);
		}
	}
