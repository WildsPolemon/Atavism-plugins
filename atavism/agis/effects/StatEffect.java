...
public class StatEffect extends AgisEffect {
    ...
    //	CombatInfo caster = state.getSource();
    	CombatInfo target = state.getTarget();
    	int skillLevel = (int) params.getOrDefault("skillLevel",0);
    	int stackLevel = state.getCurrentStack();
...
//		}
    	...
    	for (Map.Entry<String, Float> entry : statMap.entrySet()) {
    		...
    		if(Log.loggingDebug)
    				Log.debug("STATEFFECT: statModifier: " + statModifier);
    		if (modifyPercentage) {
    			target.statReapplyPercentModifier(entry.getKey(), state, statModifier, false);
    		} else {
    			target.statReapplyModifier(entry.getKey(), state, (int)statModifier, false);
    		}
    	}
    	target.statSendUpdate(false);
    }
