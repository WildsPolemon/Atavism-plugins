...
public class AttackDotEffect extends AgisEffect {
	...
	public void apply(EffectState state) {
		...
		int hitRoll = (int) params.get("hitRoll");
		...
		if (target != null) {
			...
			for (EffectState existingState : target.getCurrentEffects()) {
				if (existingState.getEffect().getID() == getID() && !state.equals(existingState)) {
					...
					damageFromEffects += pde.getPulseDamage(existingState);
					hasThisEffect = true;
					similarEffect = existingState;
					if (source.getOwnerOid().equals(similarEffect.getStackCaster())) {
						damageFromEffectsSame += pde.getPulseDamage(similarEffect);
						fromThisCaster = true;
		...
		if (stackCase == 1) {
			...
			if (fromThisCaster) {
				...
				se.setPulseDamage(sameCasterEffect,pulseDamage * stackLevel);
			...
			if (hasThisEffect) {
				...
				se.setPulseDamage(similarEffect,pulseDamage * stackLevel);
				state.wasApplied(false);
				return;
			}
		} else if (stackCase == 5) {
		...
		params.put("pulseDamage", pulseDamage);
	...
	public void pulse(EffectState state) {
		...
		int pulseDamage = (int) params.get("pulseDamage");
		int claimID = -1;
		int objectID = -1;
		if (params.containsKey("claimID"))
			claimID = (int) params.get("claimID");
	...
	public int getPulseDamage(EffectState state) {
		if (state != null) {
			return (int) state.getParams().getOrDefault("pulseDamage", 0);
		}
		return 0;
	}

	public void setPulseDamage(EffectState state, int value) {
		if (state != null) {
			state.getParams().put("pulseDamage", value);
		}
	}
