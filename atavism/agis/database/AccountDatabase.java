...
public class AccountDatabase {
	 ...
	 * @param rankingId
	 */
	public void deleteCollectionData(int rankingId) {
		Log.debug("deleteCollectionData: start");
		try {
			String updateString = "DELETE FROM `achivement_data` WHERE rankingId = "+ rankingId;
	...
	/**
	 * Function to delete ranking data
	 * @param ranking
	 */
	public void deleteRanking(int ranking) {
		Log.debug("deleteRanking: start");
		try {
			String updateString = "DELETE FROM `rankings` WHERE ranking = "+ranking;
			if(Log.loggingDebug)	Log.debug("deleteRanking: Delete sql: " + updateString);
			queries.executeUpdate(updateString);
		} catch (Exception e) {
			Log.dumpStack(e.getMessage());
		}
		Log.debug("deleteRanking: end  ");
	...
	public void deletePlayerCollectionData(OID playerOid) {
		Log.debug("deletePlayerCollectionData: start");
		try {
			String updateString = "DELETE FROM `achivement_data` WHERE playerOid = "+playerOid.toLong();
			if(Log.loggingDebug)	Log.debug("deletePlayerCollectionData: Delete sql: " + updateString);
			queries.executeUpdate(updateString);
		} catch (Exception e) {
			Log.dumpStack(e.getMessage());
		}
		Log.debug("deletePlayerCollectionData: end  ");
	...
	public LinkedList<Cooldown> getCooldowns(OID characterOID) {
		...
		if (Log.loggingDebug)Log.debug("Character : Cooldowns character oid: " + characterOID + " " + characterOID.toLong());
		...
		try (PreparedStatement ps = queries.prepare("SELECT * FROM `cooldowns` where obj_oid=" + characterOID.toLong())) {
    			...
    			if (rs != null) {
    				...
    				while (rs.next()) {
    					...
    					if (startTime + duration > System.currentTimeMillis()) {
							...
							if(!cooldownList.contains(c)) {
								if (Log.loggingDebug)Log.debug(" cooldown added id " + rs.getInt("id"));
								cooldownList.add(c);
							}else{
								if (Log.loggingDebug)Log.debug(" cooldown not added  id " + rs.getInt("id"));
							}
    					} else {
							if (Log.loggingDebug)Log.debug("Delete cooldown id " + rs.getInt("id"));
			...
			if (Log.loggingDebug)	Log.debug("Character : Cooldowns character oid: " + characterOID + " " + cooldownList);
	...
	public HashMap<String, Serializable> loadInstancePrefabData() {
		...
		try (PreparedStatement ps = queries.prepare("SELECT * FROM instance_template")) {
			try (ResultSet rs = queries.executeSelect(ps)) {
				if (rs != null) {
					while (rs.next()) {
						list.put("i"+c+"i",rs.getInt("id"));
						list.put("i"+c+"n",HelperFunctions.readEncodedString(rs.getBytes("island_name")));
						list.put("i"+c+"t",rs.getInt("islandType"));
						c++;
					}
				}
			}
			...
			e.printStackTrace();
			Log.dumpStack("loadInstancePrefabData Exception " + e);
		}
		list.put("num",c);
		return list;
	}
	...
	public boolean isPlayerIsInGuild(OID player){
		try {
			try (PreparedStatement ps = queries.prepare("SELECT * FROM `guild_member` WHERE memberOid = " + player.toLong() )) {
				try (ResultSet rs = queries.executeSelect(ps)) {
					if (rs != null) {
						while (rs.next()) {
							return true;
			...
			Log.error("isPlayerIsInGuild: SQLException: " + e.getLocalizedMessage());
		}
		return false;
	}
