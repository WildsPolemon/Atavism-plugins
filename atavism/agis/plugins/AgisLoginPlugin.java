	...
	protected CharacterResponseMessage handleCharacterRequestMessage(CharacterRequestMessage message, SocketHandler clientSocket) {
		...
		for (OID oid : charIds) {
			...
			Log.debug("LOGIN: props: " + entity.getPropertyMap());
				entity.getPropertyMapRef().forEach((k,v) -> {
					Log.debug("LOGIN: prop: " + k + "=" + v);
				});
			...
			if (cooldownList.size() > 0) {
				...
				Cooldown.activateCooldowns(cooldownList, entity2, cooldownGlobalMod, cooldownWeaponMod, cooldownMod, entity2.statGetBaseValue(CombatPlugin.ATTACK_SPEED_STAT), false, false);
