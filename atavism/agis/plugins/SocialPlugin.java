...
public class SocialPlugin extends EnginePlugin {
	...
	class PrivateInviteHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if(Log.loggingDebug)log.debug("PrivateInviteHook oid=" + oid + " targetOid=" + targetOid);
			...
			if (plyInst == null) {
				if(Log.loggingDebug)log.debug("PrivateInviteHook: playerOid of the instance is null");
				ExtendedCombatMessages.sendErrorMessage(oid, "You can only invite when you are in a private instance.");
				return true;
			}
			if (!plyInst.equals(oid)) {
				if(Log.loggingDebug)log.debug("PrivateInviteHook: playerOid of the instance " + plyInst + " is not equals " + oid);
	...
	class PrivateInviteResponseHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if(Log.loggingDebug)log.debug("PrivateInviteHook responder=" + responder + " inviter=" + inviter + " response=" + response);

			if (response == true) {
				BasicWorldNode player = WorldManagerClient.getWorldNode(inviter);
				InstanceInfo instInfo = InstanceClient.getInstanceInfo(player.getInstanceOid(), InstanceClient.FLAG_ALL_INFO);
				if(Log.loggingDebug)log.debug("PrivateInviteHook: inctanceType="+instInfo.inctanceType+" inviter="+inviter+" instInfo.playerOid="+instInfo.playerOid);
	...
	class LogoutHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if(Log.loggingDebug)log.debug("LOGOUT: SocialPlugin logout started for: " + playerOid);
			...
			if(Log.loggingDebug)log.debug("LOGOUT: SocialPlugin logout finished for: " + playerOid);
			return true;
		}
	}
