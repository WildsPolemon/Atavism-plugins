...
public class PrefabPlugin extends EnginePlugin implements TcpAcceptCallback {
	...
	//	if (iDB == null)
		...
		// Load Prefabs Data
		prefabsData.put("Instances", aDB.loadInstancePrefabData());
		prefabsData.put("GameSettings", cDB.loadGameSettings());
		...
		prefabsData.put("PvpRanks", ctDB.LoadPvpRanksPrefabData());
		prefabsData.put("Faction", mDB.LoadFactionPrefabData());
	...
	protected GetPrefabResponseMessage handleGetPrefabMessage(GetPrefabMessage message, SocketHandler clientSocket) {
		...
		} else if (message.getMsgtype().equals("PvpRanks")) {
			props.putAll(loadPvpRanksPrefabData(message, clientSocket));
			response.setType("PvpRanks");
		}else if (message.getMsgtype().equals("Faction")) {
			props.putAll(loadFactionPrefabData(message, clientSocket));
			response.setType("Faction");
		}else if (message.getMsgtype().equals("Settings")) {
			props.putAll(loadSettingsData(message, clientSocket));
			response.setType("Settings");
		}else if (message.getMsgtype().equals("Instances")) {
			props.putAll(loadInstancesData(message, clientSocket));
			response.setType("Instances");
		}
	...
	private Map<String,Serializable> loadInstancesData(GetPrefabMessage message, SocketHandler clientSocket) {
		HashMap<String, Serializable> props = new HashMap<String, Serializable>(prefabsData.get("Instances"));
		return props;
	}

	private Map<String,Serializable> loadSettingsData(GetPrefabMessage message, SocketHandler clientSocket) {
        HashMap<String, Serializable> props = new HashMap<String, Serializable>(prefabsData.get("GameSettings"));
		return props;
	}
	...
	protected Map<String, Serializable> getCountData(GetPrefabMessage message, SocketHandler clientSocket) {
		...
		prefabData = prefabsData.get("PvpRanks");
		numData = (int) prefabData.get("num");
		props.put("PvpRanks", numData);
		prefabData = prefabsData.get("Faction");
		numData = (int) prefabData.get("num");
		props.put("Faction", numData);
	...
	protected Map<String, Serializable> loadGameSettingsPrefabData(GetPrefabMessage message, SocketHandler clientSocket){
		...
		props.put("num", c);
		props.putAll(prefabsData.get("GameSettings"));
	...
	/**
	 * Prepare response data of the faction profile definition
	 * @param message
	 * @param clientSocket
	 * @return
	 */
	protected Map<String, Serializable> loadFactionPrefabData(GetPrefabMessage message, SocketHandler clientSocket) {
		log.debug("loadFactionPrefabData: Start");
		HashMap<String, Serializable> props = new HashMap<String, Serializable>();
		int num = (int) message.getProperties().get("c");
		HashMap<Integer, Long> plyItems = new HashMap<Integer, Long>();
		for (int i = 0; i < num; i++) {
		...
		if (Log.loggingDebug)
			log.debug("loadFactionPrefabData: objs=" + plyItems);
		HashMap<Integer, Long> objs = new HashMap<Integer, Long>(plyItems);
		HashMap<String, Serializable> prefabData = prefabsData.get("Faction");
		if (Log.loggingDebug)
			Log.debug("loadFactionPrefabData plyItems=" + plyItems.size() + " prefabData=" + prefabData.size());
		int numData = (int) prefabData.get("num");
		if (Log.loggingDebug) Log.debug("loadFactionPrefabData numData=" + numData);
		...
		try {
			for (int i = 0; i < numData; i++) {
				...
				if (plyItems.containsKey(id)) {
					objs.remove(id);
					if (date != plyItems.get(id)) {
						if (Log.loggingDebug) Log.debug("loadFactionPrefabData: id=" + id + " newer date");
						props.put("i" + c + "id", id);
						props.put("i" + c + "date", date);
						props.put("i" + c + "name", prefabData.get("i" + i + "name"));
						props.put("i" + c + "color", prefabData.get("i" + i + "color"));
...
//						props.put("i" + c + "num", sNum);
					...
					if (Log.loggingDebug) Log.debug("loadFactionPrefabData: id=" + id + " new ");
					props.put("i" + c + "id", id);
					props.put("i" + c + "date", date);
					props.put("i" + c + "name", prefabData.get("i" + i + "name"));
					props.put("i" + c + "color", prefabData.get("i" + i + "color"));
...
//					props.put("i" + c + "num", sNum);
				...
				if (c == PrefabPlugin.PREFAB_LOAD_ELEMENT_LIMIT) {
					props.put("all", false);
					props.put("num", c);
					if (Log.loggingDebug) log.debug("loadFactionPrefabData: end send items " + c);
					return props;
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Prepare response data of the pvp ranks profile definition
	 * @param message
	 * @param clientSocket
	 * @return
	 */
	protected Map<String, Serializable> loadPvpRanksPrefabData(GetPrefabMessage message, SocketHandler clientSocket) {
		log.debug("loadPvpRanksPrefabData: Start");
		HashMap<String, Serializable> props = new HashMap<String, Serializable>();
		int num = (int) message.getProperties().get("c");
		HashMap<Integer, Long> plyItems = new HashMap<Integer, Long>();
		for (int i = 0; i < num; i++) {
		...
		if (Log.loggingDebug)
			log.debug("loadPvpRanksPrefabData: objs=" + plyItems);
		HashMap<Integer, Long> objs = new HashMap<Integer, Long>(plyItems);
		HashMap<String, Serializable> prefabData = prefabsData.get("PvpRanks");
		if (Log.loggingDebug)
			Log.debug("loadPvpRanksPrefabData plyItems=" + plyItems.size() + " prefabData=" + prefabData.size());
		int numData = (int) prefabData.get("num");
		if (Log.loggingDebug) Log.debug("loadPvpRanksPrefabData numData=" + numData);
		...
		try {
			for (int i = 0; i < numData; i++) {
				...
				if (plyItems.containsKey(id)) {
					objs.remove(id);
					if (date != plyItems.get(id)) {
						if (Log.loggingDebug) Log.debug("loadPvpRanksPrefabData: id=" + id + " newer date");
						props.put("i" + c + "id", id);
						props.put("i" + c + "date", date);
						props.put("i" + c + "title", prefabData.get("i" + i + "title"));
						props.put("i" + c + "v", prefabData.get("i" + i + "v"));
//						props.put("i" + c + "dfi", prefabData.get("i" + i + "dfi"));
//						props.put("i" + c + "dt", prefabData.get("i" + i + "dt"));
//						props.put("i" + c + "color", prefabData.get("i" + i + "color"));
//						props.put("i" + c + "dpri", prefabData.get("i" + i + "dpri"));
						props.put("i" + c + "icon", prefabData.get("i" + i + "icon"));
						props.put("i" + c + "icon2", prefabData.get("i" + i + "icon2"));
...
//						props.put("i" + c + "num", sNum);
					...
					if (Log.loggingDebug) Log.debug("loadPvpRanksPrefabData: id=" + id + " new ");
					props.put("i" + c + "id", id);
					props.put("i" + c + "date", date);
					props.put("i" + c + "title", prefabData.get("i" + i + "title"));
					props.put("i" + c + "v", prefabData.get("i" + i + "v"));
...
//					props.put("i" + c + "num", sNum);
				...
				if (c == PrefabPlugin.PREFAB_LOAD_ELEMENT_LIMIT) {
					props.put("all", false);
					props.put("num", c);
					if (Log.loggingDebug) log.debug("loadPvpRanksPrefabData: end send items " + c);
					return props;
				}
			}
		} catch (Exception e) {
...
//						props.put("i" + c + "id3",prefabData.get("i" + i + "id3"));
						...
						props.put("i" + c + "r",prefabData.get("i" + i + "r"));
				...
				if (date != plyItems.get(id)) {
					...
					for (int k = 0; k < rewardLevel; k++) {
						...
						for (int j = 0; j < rewardsC; j++) {
							props.put("i" + c + "rewardCh" + k + "_" + j, prefabData.get("i" + i + "rewardCh" + k + "_" + j));
							props.put("i" + c + "rewardChc" + k + "_" + j, prefabData.get("i" + i + "rewardChc" + k + "_" + j));
						}
					}
