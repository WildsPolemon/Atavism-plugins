...
public class FactionClient {
	...
	public static class getStanceMessage extends PropertyMessage {
        ...
        public getStanceMessage(OID oid, LinkedList<OID> targetOid) {
            ...
            if(Log.loggingDebug)Log.debug("FactionClient: getStanceMessage  targetOids "+targetOid);
        }
        ...
        public getStanceMessage(OID oid, OID targetOid) {
            ...
            if(Log.loggingDebug)Log.debug("FactionClient: getStanceMessage  targetOid "+targetOid);
        }
        public getStanceMessage(OID oid, int faction) {
        	super(oid);
        	setFaction(faction);
        	setMsgType(MSG_TYPE_GET_STANCE);
            if(Log.loggingDebug)Log.debug("FactionClient: getStanceMessage Faction stance "+faction);
        }
    ...
    // get a mobs reaction towards a player
    public static void alterFaction(OID oid, int faction) {
        AlterFactionMessage msg = new AlterFactionMessage(oid, faction);
        Engine.getAgent().sendBroadcast(msg);
    }

    public static class AlterFactionMessage extends PropertyMessage {
        private static final long serialVersionUID = 1L;

        public AlterFactionMessage() {
            super();
        }

        public AlterFactionMessage(OID oid, int faction) {
            super(oid);
            setFaction(faction);
            setMsgType(MSG_TYPE_ALTER_FACTION);
            Log.debug("FACTION CLIENT: AlterReputationMessage hit 1");
        }
    ...
    public static final MessageType MSG_TYPE_ALTER_FACTION = MessageType.intern("faction.ALTER_FACTION");
    public static final MessageType MSG_TYPE_UPDATE_PVP_STATE = MessageType.intern("faction.UPDATE_PVP_STATE");
    public static final MessageType MSG_TYPE_PVP_REGION = MessageType.intern("faction.PVP_REGION");
