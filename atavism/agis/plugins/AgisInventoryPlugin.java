...
public class AgisInventoryPlugin extends InventoryPlugin {
    ...
    public void onActivate() {
        ...
        getHookManager().addHook(AgisInventoryClient.MSG_TYPE_ALTER_CURRENCY_ALL_PLAYERS, new AlterCurrencyAllPlayersHook());
        ...
        //  filterNoResponse.addType(AgisInventoryClient.MSG_TYPE_REMOVE_SPECIFIC_ITEM);
            ...
            filterNoResponse.addType(AgisInventoryClient.MSG_TYPE_ALTER_CURRENCY_ALL_PLAYERS);
    ...
    void loadData() {
        ...
        String useVoxelsToCheckMobVisibility = cDB.loadGameSetting("USE_VOXELS_TO_CHECK_MOB_VISIBILITY");
        if (useVoxelsToCheckMobVisibility != null) {
            MobManagerPlugin.useVoxelsToCheckMobVisibility = Boolean.parseBoolean(useVoxelsToCheckMobVisibility);
            Log.debug("Game Setting USE_VOXELS_TO_CHECK_MOB_VISIBILITY set to: " + useVoxelsToCheckMobVisibility);
        }
		...
		if (ai.getIntProperty("enchantLevel") > 0) {
			...
			if (ep.GetLevels().containsKey(ai.getIntProperty("enchantLevel"))) {
				for (int e = 1; e <= ai.getIntProperty("enchantLevel"); e++) {
					enchantStats.replace(CombatPlugin.GEAR_SCORE,enchantStats.get(CombatPlugin.GEAR_SCORE)+ep.GetLevels().get(e).GetGearScoreValue()+(int)(enchantStats.get("gearScore") * ep.GetLevels().get(e).GetGearScoreValuePercentage()));
...
//                        }
                ...
                for (String effect : effects) {
                    ...
                    cE.putArgument("length", effectTime/1000f);
...
//                }
     ...
     * @return
     */
    public static Bag getBag(OID oid, boolean loadIfNotFound ) {
        Bag b = (Bag) EntityManager.getEntityByNamespace(oid, Namespace.BAG);
        if (oid != null && b == null && loadIfNotFound) {
...
public static void updatePetList(OID ownerOid) {
   ...
   public static void updatePetList(InventoryInfo iInfo) {
    ...
    protected void createBags(InventoryInfo iInfo, OID mobOid, boolean isPlayer) {
        ...
        subBag.setPersistenceFlag(isPlayer);
     ...
     * @return
     */
    public static Bag createSubBag(OID ownerOid, int useType, int numSlots, String name) {
        // create the bag
        Bag bag = new Bag();
...
//      Log.error("sendInvUpdate: sendBagInvUpdate after");
    ...
    protected void sendBagInvUpdate(OID mobOid) {
        ...
        props.put("numBags", bags.length);
        HashMap<Integer, Integer> itemSets = getEquipSetsInfo(mobOid);
        for (int bagPos = 0; bagPos < bags.length; bagPos++) {
            OID subBagOid = bags[bagPos];
            if (Log.loggingDebug)
                log.debug("sendBagInvUpdate: bagPos=" + bagPos + ", subBagOid=" + subBagOid );
            if (subBagOid == null) {
            ...
            for (int itemPos = 0; itemPos < itemsInBag.length; itemPos++) {
                ...
                AgisItem item = null;
                if (Log.loggingDebug)
                    log.debug("sendBagInvUpdate: bagPos=" + bagPos + ", bagPos=" + itemPos + " itemOid=" + oid);
                if (oid == null) {
                ...
                if (Log.loggingDebug)
                    log.debug("sendBagInvUpdate: bagPos=" + bagPos + ", bagPos=" + itemPos + " itemOid=" + oid + " item=" + item + " itemExists=" + itemExists);
                if (itemExists) {
                    if (Log.loggingDebug)
                        log.debug("sendBagInvUpdate: adding bagNum=" + bagPos + ", bagPos=" + itemPos + ", itemOid=" + oid + ", itemName=" + item.getName() + ",icon=" + item.getIcon());
...
//        log.error("sendEquippedInvUpdate: equip="+Arrays.toString(eitems));
        ...
        for (int pos = 0; pos < subBag.getNumSlots(); pos++) {
            ...
            if (item.getProperty("enchantStats") != null) {
                ...
                for (String statName : stats.keySet()) {
                    if (bstats.containsKey(statName)) {
                        if(Log.loggingDebug)log.debug("sendEquippedInvUpdate"+ item.getName()+" "+statName+" "+stats.get(statName)+" "+bstats.get(statName)+" "+(stats.get(statName) - bstats.get(statName))+" ?");
                        if (stats.get(statName) - bstats.get(statName) != 0) {
        ...
        ammoLoaded = (Integer)iInfo.getProperty(CombatInfo.COMBAT_AMMO_LOADED);
        if(ammoLoaded==null)
        ...
        if (ammoLoaded != null) {
            ...
            iInfo.setProperty(CombatInfo.COMBAT_AMMO_LOADED,ammoLoaded);
        }
    ...
    class GetSpecificItemDataHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            for (int bagPos = 0; bagPos < items.length; bagPos++) {
                ...
                if (subBagOid == null) {
                    log.error("GetSpecificItemDataHook: sub bag oid is null");
                    continue;
                }
                Bag subBag = getBag(subBagOid);
                if (subBag == null) {
                    log.error("GetSpecificItemDataHook: sub bag obj is null");
                    continue;
                }
                //sendInvUpdateHelper(invUpdateMsg, pos, subBag);
                OID[] itemsInBag = subBag.getItemsList();
                for (int itemPos = 0; itemPos < itemsInBag.length; itemPos++) {
                    ...
                    AgisItem item = getAgisItem(oid);
                    if (item == null) {
                        Log.warn("GetSpecificItemDataHook: item is null, oid=" + oid);
                        itemExists = false;
                    }
                    if (itemExists && !props.containsValue(item.getName()) && itemOids.contains(item.getOid())) {
                        if (Log.loggingDebug)
                            log.debug("GetSpecificItemDataHook: adding bagNum=" + bagPos + ", bagPos=" + itemPos +
...
//    }
     ...
     * @param value
     */
    public void setMobCurrency(OID mobOid, int currencyID, long value) {
        Currency c = Agis.CurrencyManager.get(currencyID);
        log.debug("alterMobCurrency CURRENCY: getting currency: " + currencyID+" delta: "+value);
        if (c.getExternal()) {
            OID accountID = Engine.getDatabase().getAccountOid(mobOid);
            authDB.alterAccountCoinAmount(accountID, value);
            InventoryInfo iInfo = getInventoryInfo(mobOid);
            ExtendedCombatMessages.sendCurrencies(mobOid, iInfo.getCurrentCurrencies());
        }else {
            InventoryInfo iInfo = getInventoryInfo(mobOid);
            ...
            if (iInfo != null) {
                iInfo.getCurrencies(iInfo.getCurrentCategory()).put(currencyID, value);
                Engine.getPersistenceManager().setDirty(iInfo);
                ExtendedCombatMessages.sendCurrencies(mobOid, iInfo.getCurrentCurrencies());
            }else {
                log.error("alterMobCurrency mobOid="+mobOid+" InventoryInfo is null");
    ...
    class AlterCurrencyHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            if(alterMsg.isSetValue()){
                setMobCurrency(oid,currencyType,delta);
            }else {
                alterMobCurrency(oid, currencyType, delta);
            }
            alterCurrecyLog(oid,null, currencyType, delta,"alterCurrencyMessage setvalue "+alterMsg.isSetValue());

            //Check PVP Rank
            AtomicBoolean found = new AtomicBoolean(false);
            PvpRank prev =null;

//            Iterator<Map.Entry<Integer, PvpRank>> it = CombatPlugin.PVP_RANKS.entrySet().iterator();
//            AtavismDatabase atDB = new AtavismDatabase();
//            HashMap<Integer, Integer> rankCount = atDB.getRanksCount();
//            while (it.hasNext()) {
//                Map.Entry<Integer, PvpRank> entry = it.next();
//                prev= entry.getValue();
//
//            }
            ...
            CombatPlugin.PVP_RANKS.forEach((k, v) -> {
                long currency = getMobCurrency(oid, v.getCurrency());
                if(v.getValue() <= currency && !found.get()) {
                    found.set(true);
                    HashMap<String, Serializable> param = new HashMap<>();
                    param.put(FactionStateInfo.PVP_RANK_PROP,k);
                    param.put("pvpRankName", v.getTitle());
                    CombatClient.setCombatInfoParamsNoResponse(oid, param);
                    //Send Update to Client
                    WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(oid, oid);
                    tpm.setProperty("pvpRankName", v.getTitle());
                    tpm.setProperty(FactionStateInfo.PVP_RANK_PROP, k);
                    Engine.getAgent().sendBroadcast(tpm);
                }
            });
//            AgisInventoryClient.sendCurrencyAltered(oid, currencyType, getMobCurrency(oid, currencyType));
            if (Log.loggingDebug)
     ...
     *
     */
    class AlterCurrencyAllPlayersHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            GenericMessage alterMsg = (GenericMessage) msg;
            int currencyType = (int) alterMsg.getProperty("currencyType");
            Long value = (Long) alterMsg.getProperty("value");

            if (Log.loggingDebug)
                log.debug("AlterCurrencyAllPlayersHook:  currencyType:" + currencyType + "; To value:" + value);

           Entity[] entities =  EntityManager.getAllEntitiesByNamespace(Namespace.BAG);

           for (Entity entity : entities) {
               if(entity instanceof InventoryInfo) {
                   setMobCurrency(entity.getOid(), currencyType, value);
                   alterCurrecyLog(entity.getOid(), null, currencyType, value, "AlterCurrencyAllPlayersHook");
                   //Check PVP Rank
                   AtomicBoolean found = new AtomicBoolean(false);
                   CombatPlugin.PVP_RANKS.forEach((k, v) -> {
                       long currency = getMobCurrency(entity.getOid(), v.getCurrency());
                       if (v.getValue() <= currency && !found.get()) {
                           found.set(true);
                           HashMap<String, Serializable> param = new HashMap<>();
                           param.put("pvpRank", k);
                           param.put("pvpRankName", v.getTitle());
                           CombatClient.setCombatInfoParamsNoResponse(entity.getOid(), param);
                           //Send Update to Client
                           WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(entity.getOid(), entity.getOid());
                           tpm.setProperty("pvpRankName", v.getTitle());
                           tpm.setProperty("pvpRank", k);
                           Engine.getAgent().sendBroadcast(tpm);
                       }
                   });
               }
           ...
           db.alterCurrencyForAllPlayers( currencyType, value);
//            AgisInventoryClient.sendCurrencyAltered(oid, currencyType, getMobCurrency(oid, currencyType));
            if (Log.loggingDebug)
                log.debug("AlterCurrencyAllPlayersHook:  END");
            return true;
        }
    }
    ...
    */
    ...
    protected void loadInventory(Entity e) {
        try {
            ...
            for (int i = 0; i < bags.length; i++) {
                OID sbo = bags[i];
                Bag b = getBag(sbo);
                OID[] list = b.getItemsList();
                for (int j = 0; j < list.length; j++) {
                    if(list[j]!=null){
                        AgisItem item = getAgisItem(list[j]);
                        AgisInventoryClient.itemAcquiredStatusChange(ownerOid, item, true);
                    }
                }
            }
           ...
           for(PetInfo pi : pets){
               if (Log.loggingDebug) Log.debug("loadInventory: "+pi);
    ...
    protected void unloadInventory(Entity e) {
        ...
        for (OID subBagOid : iInfo.getBags()) {
            if (subBagOid != null) {
                Bag subBag = getBag(subBagOid, false);
                if (subBag == null)
                    continue;
                if (subBag.getPersistenceFlag()) {
                    Engine.getPersistenceManager().persistEntity(subBag);
                }
                for (OID itemOid : subBag.getItemsList()) {
                    if (itemOid == null)
                        continue;
        ...
        if (iInfo.getEquipmentItemBag() != null) {
            Bag subBag = getBag(iInfo.getEquipmentItemBag(), false);
            if (subBag == null)
                return;
            if (subBag.getPersistenceFlag()) {
                Engine.getPersistenceManager().persistEntity(subBag);
        ...
        if(Log.loggingDebug)Log.debug("unloadInventory: oid="+e.getOid()+" owner="+ownerOid+" END");
    }
    ...
    protected boolean saveInventory(Entity e, Namespace namespace) {
        ...
        if (e instanceof InventoryInfo) {
            ...
            // Save Storage Bags
            for (OID bagOid : iInfo.getStorageBags().values()) {
                Bag storageBag = getBag(bagOid, false);
                if (storageBag != null) {
                    if (Log.loggingDebug)
                        ...
                        if (m.getItems().get(i) != null) {
                            ...
                            if (item != null) {
                                ...
                                if (ObjectManagerClient.loadObject(m.getItems().get(i)) == null) {
                                    if (Log.loggingDebug)
                                        Log.debug("MAIL: got null item to save: " + m.getItems().get(i) + " in mail: " + m.getID());
                                } else {
                                    ObjectManagerClient.saveObject(m.getItems().get(i));
                                }
                            }
        ...
        if(e instanceof Bag){
            if (Log.loggingDebug)
                log.debug("saveInventory: Bag=" + e.getOid());
            Engine.getDatabase().saveObject(e, Namespace.BAG);
        }
    ...
    protected boolean addItem(OID mobOid, OID rootBagOid, OID itemOid, boolean tryStack) {
        ...
        try {
            ...
            if (Log.loggingDebug)
                log.debug("addItem: InventoryInfo: " + iInfo);
            if(iInfo==null){
    ...
    private void sendLootGroundUpdate(OID target, OID instanceOid) {
            ...
            props.put("num", c.get());
            if(Log.loggingDebug)
                log.debug("sendLootGroundUpdate target="+target+" " + props);
   ...
   private LinkedList<OID> CalculateLootRoll(OID mobOid,OID itemOid,LinkedList<OID> loot) {
    ...
    class LootChestHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            if(Log.loggingDebug)  log.debug("LOOT: removed oid=" + itemOid);
            
            // add item to the looter's root bag
            boolean rv = addItem(looterOid, looterOid, itemOid);
            if(Log.loggingDebug) log.debug("LOOT: addItem to looter, oid=" + itemOid + ", rv=" + rv);
            if (!rv) {
                DataLoggerClient.logData("ITEM_LOOTED_FAILED_EVENT", looterOid, itemOid, accountId, logData);
    ...
    class EquipItemInSlotHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
         ...
         /*   if (AgisEquipSlot.getSlotByName(slotName) == null) {
            ...
            RequirementCheckResult canUse = RequirementChecker.DoesPlayerMeetRequirements(mobOid, requirements);
            if (!canUse.result.equals(RequirementCheckResult.RESULT_SUCCESS)) {
                EventMessageHelper.SendRequirementFailedEvent(mobOid, canUse);
                return false;
            }
    ...
    class EquipItemInPetSlotHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            if(Log.loggingDebug) Log.debug("EquipItemInPetSlotHook PetInfo " + pi);
            PetProfile pp = Agis.PetProfile.get(profile);
            if(Log.loggingDebug)Log.debug("EquipItemInPetSlotHook PetProfile " + pp);
            ...
            equipItem(item, ownerOid, true, slot, profile, pi);
    ...
    public boolean equipItem(AOObject itemObj, OID activatorOid, boolean replace) {
        return equipItem(itemObj, activatorOid, replace, null,-1, null);
    }
    public boolean equipItem(AOObject itemObj, OID activatorOid, boolean replace, AgisEquipSlot targetSlot) {
        return equipItem(itemObj, activatorOid, replace, targetSlot,-1, null);
    }
     ...
     */
    public boolean equipItem(AOObject itemObj, OID activatorOid, boolean replace, AgisEquipSlot targetSlot, int petProfile , PetInfo pi ) {
        if (Log.loggingDebug)
        ...
        EquipMap eqm;
        if(petProfile > 0){
            eqm = iInfo.getPetInventory().get(petProfile).getEquipMap();
        } else {
            eqm = getEquipMap(activatorOid);
        }

        boolean itemFoundInEquipment = false;
        if(eqm.containsValue(item.getOid()))
            itemFoundInEquipment = true;

        if (!itemFoundInBag) {
            log.error("AgisInventoryPlugin.equipItem: item=" + item + ", activatorOid=" + activatorOid + " itemInBag=" + itemInBag + " itemFoundInBag=" + itemFoundInBag + " itemFoundInEquipment="+itemFoundInEquipment+" not found reference in bag break");
            return false;
        }

        // TODO: Add checking requirements for pet
        if (petProfile > 0) {
          PetInventoryInfo pii = iInfo.getPetInventory().get(petProfile);
          ArrayList<OID> pets = pii.getPets();
          if(!pets.isEmpty()) {
              OID pOid = pets.get(0);
              HashMap<Integer, HashMap<String, Integer>> requirements = (HashMap<Integer, HashMap<String, Integer>>) item.getProperty("requirements");
              RequirementCheckResult canUse = RequirementChecker.DoesPlayerMeetRequirements(pOid, requirements, true, pi);
              if (!canUse.result.equals(RequirementCheckResult.RESULT_SUCCESS)) {
                  EventMessageHelper.SendRequirementFailedEvent(activatorOid, canUse);
                  log.debug("AgisInventoryPlugin.equipItem: pet Requirement failed");
                  return false;
              }
              log.debug("AgisInventoryPlugin.equipItem: pet Requirement OK");
          } else {
              log.debug("AgisInventoryPlugin.equipItem: pets list is empty");
//              EventMessageHelper.SendRequirementFailedEvent(activatorOid, canUse);
              return false;
          }
        }
...
//            eitems = equipBag.getItemsList();
            ...
            if (ammoTypeReq != null) {
                ...
                if (ammoID != null && ammoID > 0) {
                    ...
                    if (ammoType != ammoTypeReq) {
                        ...
                        iInfo.setProperty(CombatInfo.COMBAT_AMMO_LOADED, -1);
                    }
                }
            }
    ...
    public boolean removeEquippedItem(OID activatorOid, OID itemOid, AgisEquipSlot slot, int petProfile) {
        if (Log.loggingDebug)
            log.debug("removeItemFromBagHelper: activatorOid=" + activatorOid + " itemOid="+itemOid+" slot="+slot+" petProfile="+petProfile);
        InventoryInfo iInfo = getInventoryInfo(activatorOid);
        ...
        if(!itemOid.equals(sItemOid)){
            ...
            if(subBagOid!=null && item.getProperty(INVENTORY_PROP_BACKREF_KEY) != null && subBagOid.equals(item.getProperty(INVENTORY_PROP_BACKREF_KEY))){
                if (Log.loggingDebug)
                    log.debug("removeItemFromBagHelper: item itemOid=" + item.getOid() + " has reference to bagOid="+subBagOid);
                Integer index = subBag.findItem(itemOid);
                if(index != null && index >= 0){
                    if (Log.loggingDebug)
                        log.debug("removeItemFromBagHelper: item  itemOid=" + item.getOid() + " bagOid="+subBagOid + " not in slot "+slot+" its in slot index "+index+" "+AgisEquipSlot.getSlotById(index));
                } else {
                    if (Log.loggingDebug)
                        log.debug("removeItemFromBagHelper: item not in bag itemOid=" + item.getOid() + " bagOid="+subBagOid + " reset back reference ");
                    item.setProperty(INVENTORY_PROP_BACKREF_KEY, null);
                }
            }
            return false;
        ...
        public boolean processMessage(Message msg, int flags) {
            ...
            try {
                ...
                if (iInfo != null) 
                {
                    ...
                    AtomicBoolean found = new AtomicBoolean(false);
                    CombatPlugin.PVP_RANKS.forEach((k, v) -> {
                        long currency = getMobCurrency(playerOid, v.getCurrency());
                        if(Log.loggingDebug)Log.debug("LoginHook: playerOid=" + playerOid + " instanceOid=" + instanceOid +" Rank "+v+" "+v.getValue()+" currency:"+currency+" found:"+found.get());
                            if(v.getValue() <= currency && !found.get()) {
                                found.set(true);
                                HashMap<String, Serializable> param = new HashMap<>();
                                param.put(FactionStateInfo.PVP_RANK_PROP,k);
                                param.put("pvpRankName", v.getTitle());
                                CombatClient.setCombatInfoParamsNoResponse(playerOid, param);
                                //Send Update to Client
                                WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(playerOid, playerOid);
                                tpm.setProperty(FactionStateInfo.PVP_RANK_PROP, v.getTitle());
                                tpm.setProperty("pvpRank", k);
                                Engine.getAgent().sendBroadcast(tpm);
                            }
                    });
                if(Log.loggingDebug)Log.debug("LoginHook: playerOid=" + playerOid + " instanceOid=" + instanceOid +" Rank "+found.get());

            } catch (Exception e) {
                Log.exception("Inventory LoginHook", e);
            }
            if(Log.loggingDebug)Log.debug("LoginHook: playerOid=" + playerOid + " instanceOid=" + instanceOid+" End");
            Engine.getAgent().sendResponse(new ResponseMessage(message));
            return true;
        }
    }
	...
	protected void sendStorageInvUpdate(OID mobOid) {
		InventoryInfo iInfo = getInventoryInfo(mobOid);
		OID bankBagOid = iInfo.getActiveStorageBag();
		if(Log.loggingDebug)log.debug("sendStorageInvUpdate bankBagOid:" + bankBagOid + " mobOid:" + mobOid);
		if (bankBagOid == null) {
			...
			if (iInfo.getActiveStorage() != null && !iInfo.getActiveStorage().trim().isEmpty()) {
				try {
                    ...
                    if(Log.loggingDebug)  log.debug("sendStorageInvUpdate ActiveStorage is not oid "+iInfo.getActiveStorage()+" so its not sorage skip");
                }
			}
            if(Log.loggingDebug)	log.debug("sendStorageInvUpdate bankBagOid:" + bankBagOid + " mobOid:" + mobOid);
			if (bankBagOid == null) {
    ...
    protected void sendBankInvUpdate(OID mobOid, String storageKey, OID storageBagOid, boolean open) {
        if (Log.loggingDebug)Log.debug("STORAGE: sending storage inventory update for key: " + storageKey+" mobOid:"+mobOid+" storageBagOid:"+storageBagOid);
        Map<String, Serializable> props = new HashMap<String, Serializable>();
        ...
        for (int bagPos = 0; bagPos < 1; bagPos++) {
			...
			if (storageKey.startsWith(bankBagKey)) {
                if (Log.loggingDebug) log.debug("sendBankInvUpdate: Bank");
				if (subBag.getNumSlots() < BANK_SLOTS_COUNT) {
					subBag.changeNumSlots(BANK_SLOTS_COUNT);
				}
                if (Log.loggingDebug) log.debug("sendBankInvUpdate: Bank name "+subBag.getStorageName()+" <> storageKey="+storageKey);
                if(!storageKey.equals(subBag.getStorageName()))
                    subBag.setStorageName(storageKey);
                if (Log.loggingDebug)log.debug("sendBankInvUpdate: Bank Save "+subBag.getStorageName());
                Engine.getDatabase().saveObject(subBag, subBag.getNamespace());
                if (Log.loggingDebug)  log.debug("sendBankInvUpdate: Bank Save End");
			}
            //sendInvUpdateHelper(invUpdateMsg, pos, subBag);
            props.put("bag_" + bagPos + "ID", subBag.getItemTemplateID());
            props.put("bag_" + bagPos + "Name", subBag.getName());
    ...
    class StoreItemInBankHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            if (itemOid != null) {
                ...
                if (sourceBag == null) {
                    //Check if item if is in bank slot
                    Integer slotNum = bankBag.findItem(item.getOid());
                    if (slotNum != null) {
                        sourceBag = bankBag;
                        sourceSlot = slotNum;
                    }
                ...
                if (sourceBag == null) {
                    log.debug("StoreItemInBankHook: item is not in bags !!! You cant add this Item");
                    ExtendedCombatMessages.sendErrorMessage(mobOid, "You can't add this item");
                    // Somehow failed to find the bag where the equipped item is currently located
                    sendBagInvUpdate(mobOid);
                    sendStorageInvUpdate(mobOid);
                    sendInvUpdate(mobOid);
                ...
                log.debug("EnchantingDetaile: playerOid: " + playerOid + " item:" + item+" profileId:"+profileId);
                    ...
                    if (ep.GetLevels().containsKey(enchantLevel + 1)) {
                        ...
                        if(item.getPropertyMapRef().containsKey("enchantStats")) {
                            ...
                            if(item.getProperty("damage")!=null) {
...
//boolean itemAdded = addItem(playerOid, playerOid, item.getOid());
    ...
    public static void addListsRankingData(OID subjectOid, Short type, int value) {
        Log.debug("addListsRankingData "+subjectOid+" type "+type+" value="+value);
