...
import static atavism.agis.plugins.FactionClient.MSG_TYPE_PVP_REGION;
...
public class VolumetricRegion implements MessageDispatch, Serializable, MessageCallback, Runnable {
	...
	public void activate() {
		...
		mobFilterTypes.add(CombatClient.MSG_TYPE_KILLED); // S
		if(Log.loggingDebug)
	...
	public void handleMessage(Message msg, int flags) {
		...
		} else if (msg.getMsgType() == CombatClient.MSG_TYPE_KILLED) {
			PropertyMessage pMsg = (PropertyMessage) msg;
			OID killed = pMsg.getSubject();
			OID attacker = (OID) pMsg.getProperty("attacker");
			if (regionType.equals("PvP")) {
				if(Log.loggingDebug)
					Log.debug("REGION: handleMessage id=" + id + " handleMessage msg.getMsgType()=" + msg.getMsgType()+" pvp akkacker:"+attacker+" pvp_announcement="+pvp_announcement);
				if(pvp_announcement == 2){
					AccountDatabase aDB = new AccountDatabase(false);
					String plyAttacker = aDB.getCharacterNameByOid(attacker);
					String plyKilled = aDB.getCharacterNameByOid(killed);
					PropertyMessage pmsg = new PropertyMessage(ChatClient.MSG_TYPE_COM_PVP, attacker);
					String message = AgisWorldManagerPlugin.PVP_KILLED_MESSAGE.replace("{ATTACKER}", plyAttacker).replace("{KILLED}", plyKilled);
					pmsg.setProperty("message",message);
					pmsg.setProperty("channel", -20);
					Engine.getAgent().sendBroadcast(pmsg);
				}else if(pvp_announcement == 1){
					AccountDatabase aDB = new AccountDatabase(false);
					String plyAttacker = aDB.getCharacterNameByOid(attacker);
					String plyKilled = aDB.getCharacterNameByOid(killed);
					PropertyMessage pmsg = new PropertyMessage(ChatClient.MSG_TYPE_COM_PVP, attacker);
	...
	void playerEnteredRegion(OID playerOid) {
		...
		if (regionType.equals("Water")) {
			...
			if(obj.getType().equals(ObjectTypes.player)) {
				if (actionID > 0)
					CombatClient.applyEffect(playerOid, actionID);
				PropertyMessage pmsg = new PropertyMessage(MSG_TYPE_PVP_REGION,playerOid);
				pmsg.setProperty("enter", true);
				pmsg.setProperty("rType", "pvp");
				pmsg.setProperty("rMode", pvpMode );
				pmsg.setProperty("rOid", getObjectOID());
				pmsg.setProperty("iOid", getInstanceOID());
				pmsg.setProperty("pvpGroup", pvp_group);
				pmsg.setProperty("pvpGuild", pvp_guild);
				pmsg.setProperty("facs", actionData2);
				Engine.getAgent().sendBroadcast(pmsg);
				PropertyMessage propMessage = new PropertyMessage(playerOid);
				propMessage.setProperty("rpChangeName",pvp_add_rank_title_prefix);
				propMessage.setProperty("rpColorName",pvp_change_color_name);
			...
			if(obj.getType().equals(ObjectTypes.player)) {
				...
				PropertyMessage pmsg = new PropertyMessage(MSG_TYPE_PVP_REGION,playerOid);
				pmsg.setProperty("enter", true);
				pmsg.setProperty("rType", "sanctuary");
				pmsg.setProperty("rMode", pvpMode);
				pmsg.setProperty("rOid", getObjectOID());
				pmsg.setProperty("iOid", getInstanceOID());
				pmsg.setProperty("pvpGroup", pvp_group);
				pmsg.setProperty("pvpGuild", pvp_guild);
				pmsg.setProperty("sf", pvp_sanctuary_factions);
				pmsg.setProperty("facs", actionData2);
				Engine.getAgent().sendBroadcast(pmsg);
			}
		} else if (regionType.equals("Trap")) {
	...
	void playerLeftRegion(OID playerOid) {
		...
		if (regionType.equals("Water")) {
			...
			Entity entity = EntityManager.getEntityByNamespace(playerOid, Namespace.WORLD_MANAGER);
			AOObject obj = (AOObject) entity;
			if(obj!=null && obj.getType().equals(ObjectTypes.player)) {
				if (actionID > 0)
					CombatClient.removeEffect(playerOid, actionID);
				PropertyMessage pmsg = new PropertyMessage(MSG_TYPE_PVP_REGION,playerOid);
				pmsg.setProperty("enter", false);
				pmsg.setProperty("rType", "pvp");
				pmsg.setProperty("rOid", getObjectOID());
				pmsg.setProperty("iOid", getInstanceOID());
				Engine.getAgent().sendBroadcast(pmsg);
				PropertyMessage propMessage = new PropertyMessage(playerOid);
				propMessage.setProperty("rpChangeName",false);
				propMessage.setProperty("rpColorName",false);
				Engine.getAgent().sendBroadcast(propMessage);
			}
		} else if (regionType.equals("Sanctuary")) {
			Entity entity = EntityManager.getEntityByNamespace(playerOid, Namespace.WORLD_MANAGER);
	...
	public void setPvpMode(int mode) {
		this.pvpMode = mode;
	}
	...
	public void setActionData1(String actionData1) {
		...
		if(regionType.equals("PvP")) {
			if(actionData1 != null && !actionData1.isEmpty()) {
				String[] v = actionData1.split(";");
				if(v.length>0)
				pvp_guild = Objects.equals(v[0], "1");
				if(v.length>1)
					pvp_group = Objects.equals(v[1], "1");
				if(v.length>2)
					pvp_add_rank_title_prefix = Objects.equals(v[2], "1");
				if(v.length>3)
					pvp_change_color_name = Objects.equals(v[3], "1");
			}
		}

		if(Log.loggingDebug)
		Log.debug("Region "+id+" setActionData1 "+regionType+" "+actionData1+" pvp_guild="+pvp_guild+
				" pvp_group="+pvp_group+" pvp_add_rank_title_prefix="+pvp_add_rank_title_prefix+" pvp_change_color_name="+pvp_change_color_name);
	...
	public void setActionData2(String actionData2) {
		...
		if(regionType.equals("PvP")) {
			if(actionData2 != null && !actionData2.isEmpty()) {
				String[] groups = actionData2.split("\\|");
				for(String g : groups) {
					if(g != null && !g.isEmpty()) {
	...
	public void setActionData3(String actionData3) {
		...
		if(regionType.equals("PvP")) {
			if(actionData3 != null && !actionData3.isEmpty()) {
				pvp_announcement =	Integer.parseInt(actionData3);
			}
		}
	}
	...
	public void setPvpAnnouncement(int pvpAnnouncement) {
		pvp_announcement = pvpAnnouncement;
	}
	...
	int pvpMode = 0;
	boolean pvp_group =false;
	boolean pvp_guild =false;
	boolean pvp_add_rank_title_prefix =false;
	boolean pvp_change_color_name =false;
	List<List<Integer>> factionGroups = new ArrayList<>();
    // 0: none, 1: instance, 2: global
	int pvp_announcement = 2;

	String pvp_sanctuary_factions = "";
