...
public void summonPet() {
    ...
    if (displays != null && displays.size() > 0) {
        ...
        if(Log.loggingDebug)  Log.debug("MOB: got display: " + meshName);
    }
    ...
    int hitBox =  (int)tmpl.get(CombatClient.NAMESPACE, CombatPlugin.PROP_HITBOX);
    ...
    mobFactory.addBehavSupplier((obj, spawnData) -> {
        ...
        ncpBehav.setHitBoxRange(hitBox);
        return ncpBehav;
    });
    ...
    ObjectStub obj = mobFactory.makeObject(spawnData, bwNode.getInstanceOid(), destination);
    obj.spawn();
    InterpolatedWorldNode iwNode = obj.getWorldNode();

    if(Log.loggingDebug) Log.debug("PET: pet " + mobName + " spawned at: " + iwNode.getLoc() + " in instance: " + iwNode.getInstanceOid());
    if(Log.loggingDebug)  Log.debug("PET: owner is at: " + bwNode.getLoc() + " in instance: " + bwNode.getInstanceOid());

    // Set up the pet display
    String gender = "Male"; // You can modify this logic as needed
    isSpawned = true;
    mobObj = obj.getOid();
    if(Log.loggingDebug) Log.debug("NonCombatPet summonPet: Pet OID " + mobObj);
    ...
    String tempFaction = (String) EnginePlugin.getObjectProperty(ownerOid, Namespace.FACTION, FactionStateInfo.PET_FACTION_PROP);
    EnginePlugin.setObjectProperty(mobObj, Namespace.FACTION, FactionStateInfo.FACTION_PROP, faction);
    EnginePlugin.setObjectProperty(mobObj, Namespace.FACTION, FactionStateInfo.PET_FACTION_PROP, tempFaction);
    ...
    if(Log.loggingDebug)  log.debug("NonCombatPet summonPet Done " + this.despawnTime);
    if (this.despawnTime > 0) {
        if(Log.loggingDebug) log.debug("NonCombatPet summonPet scheduling despawn in " + this.despawnTime + " milliseconds");
    ...
    public boolean activate() {
     ...
     //   filter.addType(AgisMobClient.MSG_TYPE_PET_TARGET_LOST);
        ...
        if(Log.loggingDebug)  log.debug("PET: set up subscription for pet owner: " + ownerOid);
        return true;
    }
    /**
    ...
    public void handleMessage(Message msg, int flags) {
    	...
    	if (msg instanceof PropertyMessage) {
            ...
            if (playerOid.equals(ownerOid)) {
                if(Log.loggingDebug)log.debug("CombatPet owner:"+ownerOid+" is logout");
            	DespawnPet despawnPet = new DespawnPet();
            ...
            OID playerOid = logoutMsg.getSubject();
            if (playerOid.equals(ownerOid)) {
                if(Log.loggingDebug)log.debug("CombatPet owner:"+ownerOid+" left instance despawn pet");
            	DespawnPet despawnPet = new DespawnPet();
    	    	Engine.getExecutor().schedule(despawnPet, 500, TimeUnit.MILLISECONDS);
    	    	}
    ...
    class DespawnPet implements Runnable, Serializable {
    	public void run() {
    		Log.debug("PET: despawning pet: " + mobObj);
  	         if (isSpawned) {
        		try {
                    if(Log.loggingDebug)Log.debug("PET: despawning pet: " + mobObj);
        	        WorldManagerClient.despawn(mobObj);
                    if(Log.loggingDebug) Log.debug("PET: despawned pet: " + mobObj);
        		} catch (NoRecipientsException e) {
        			Log.debug("PET: no recipients found for despawn pet.");
        		}
        		isSpawned = false;
    ...
    class DistancePet implements Runnable, Serializable {
		public void run() {
            if(Log.loggingDebug)Log.debug("PET: Timer check pet:" + mobObj + " distance from owner:" + ownerOid);
            ...
            if(Log.loggingDebug)Log.debug("PET: Timer check pet:" + mobObj + " distance from owner:" + ownerOid+" distance:"+dist);
			if (dist > CombatPlugin.PET_DISTANCE_DESPAWN) {
				try {
                    if(Log.loggingDebug)Log.debug("PET: Timer distance try despawning pet: " + mobObj);
					WorldManagerClient.despawn(mobObj);
                    if(Log.loggingDebug)Log.debug("PET: Timer distance despawned pet: " + mobObj);
				} catch (NoRecipientsException e) {
                ...
                if(Log.loggingDebug)log.debug("Distance Noncombat Set hasPet false ownerOid="+ownerOid+" mobObj="+mobObj);
				}
		}
