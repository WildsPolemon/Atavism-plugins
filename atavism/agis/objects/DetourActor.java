...
public class DetourActor implements MessageDispatch {
          ...
          //  if (Log.loggingDebug)
            ...
            if (command.equals(BaseBehavior.MSG_CMD_TYPE_GOTO)) {
               ...
               // mode = BaseBehavior.MSG_CMD_TYPE_GOTO;
				...
				io.micrometer.core.instrument.Timer.builder("handle_goto").tag("handle","detour_actor")
				...
				io.micrometer.core.instrument.Timer.builder("handle_roam").tag("handle","detour_actor")
						.register(Prometheus.registry()).record(Duration.ofNanos(System.nanoTime() - tStart));
	...
	if(Log.loggingDebug)Log.debug("DetourActor.setupFollow oid"+getOid()+" TargetLoc="+prevTargetLoc+" lastTargetLoc="+lastTargetLoc+" len="+len+" distanceToFollowAt="+distanceToFollowAt+" dist="+dist);
...
//		 Point destPoint = node.getCurrentLoc();
    	 ...
    	 try {
			 ...
			 if(Log.loggingDebug)Log.debug("DetourActor.setupFollow oid"+getOid()+" TargetLoc="+prevTargetLoc+" findRandomPointAroundCircle lastTargetLoc="+lastTargetLoc);
	    ...
	    if (generatedPath == null) {
			Log.debug("DetourActor: setupFollow: invalid path");
        	AgisMobClient.sendInvalidPath(oid);
        	return;
        }
		Log.debug("DetourActor: setupFollow: generatedPath "+generatedPath);
	...
	public void updateDirLoc(AOVector dir, Point pos) {
		InterpolatedWorldNode node = (InterpolatedWorldNode) obj.getWorldNode();
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.updateDirLoc: oid = " + oid + "; mode = " + mode);
		if (mode != null && mode.equals(BaseBehavior.MSG_CMD_TYPE_FOLLOW)) {
			if(speed==0) {
				if (Log.loggingDebug)
					Log.debug("BaseBehavior.updateDirLoc: oid = " + oid + "; mode = " + mode+" speed="+speed+" return");
				return;
			}
			...
			if ((dist > 0.3f || fdist > distanceToFollowAt*0.9f )&& pdist > 0.1f) {
		    	...
		    	if (!interpolatingPath) {
					if (Log.loggingDebug)
						Log.debug("BaseBehavior.updateDirLoc: oid = " + oid + "; mode = " + mode+" interpolatingPath="+interpolatingPath+" return");
					return;
				}
				updateInterpolation();
		    }
		...
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.updateDirLoc: oid = " + oid + " END");
