...
import atavism.agis.database.AtavismDatabase;
...
//			 AgisInventoryPlugin.updatePetList(ownerOid);
	...
	public void generateEquipMapFromBag() {
		...
		if (subBag == null) {
			Log.error("PetInventoryInfo: generateEquipMapFromBag: owner " + ownerOid + " pet profile=" + petProfile);
			subBag = AgisInventoryPlugin.createSubBag(null, Bag.BAG_USE_TYPE_EQUIPPED, AgisInventoryPlugin.EQUIP_SLOTS_COUNT,"MainEquip");
			if (subBag == null) {
				return;
			}
			setEquipmentItemBag(subBag.getOid());
			AtavismDatabase atDb = new AtavismDatabase();
			atDb.savePlayerPetEquip(getOwnerOid(), getPetProfile(), subBag.getOid());
