...
public class Duel implements Serializable {
		...
		public void run() {
	    	...
	    	for (int i = 0; i < numTeams; i++) {
	    		...
	    		while (iter.hasNext()) {
	        		...
	        		EnginePlugin.setObjectProperty(oid, Namespace.FACTION, FactionStateInfo.DUEL_FACTION_PROP, factionOverride);
	    		}
	    	}
    	...
    	for (int i = 0; i < numTeams; i++) {
			for (int j = 0; j < teams[i].size(); j++) {
				if (activeFighters[i].get(j)) {
		    		...
		    		EnginePlugin.setObjectProperty(oid, Namespace.FACTION, FactionStateInfo.DUEL_FACTION_PROP, "");
				}
			}
    	}
    ...
    /**
     * Calls sendEventAll for each team in the arena.
    ...
    /**
     * Calls sendEventTeam for each player in the specified team.
     * @param eventType: the chat message to send
