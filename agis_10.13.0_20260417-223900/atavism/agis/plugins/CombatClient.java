...
public class CombatClient {
	...
	public static void PlayerKillPlayer(OID oid, OID target) {
		PropertyMessage msg = new PropertyMessage();
		msg.setMsgType(MSG_TYPE_PLAYER_KILL_PLAYER);
		msg.setSubject(oid);
		msg.setProperty("target", target);
		Engine.getAgent().sendBroadcast(msg);
	}

	public static void setTargetChaotic(OID k, Set<OID> v) {
		PropertyMessage msg = new PropertyMessage(k);
		msg.setMsgType(MSG_TYPE_TARGETS_OUTLAW);
		msg.setProperty("targets", new ArrayList<>(v));
		Engine.getAgent().sendBroadcast(msg);
		if(Log.loggingDebug)Log.debug("TARGET: sent target Outlaw message targetType=" + v +"to "+k);
	}
	...
	public static void sendPlayerWasKilled(OID oid,OID attacker) {
		if (Log.loggingDebug)
			Log.debug("send player was killed");
		PropertyMessage msg = new PropertyMessage(MSG_TYPE_KILLED, oid);
		msg.setProperty("attacker", attacker);
		Engine.getAgent().sendBroadcast(msg);
		if (Log.loggingDebug)
			Log.debug("send player was killed send");
	}
	...
	public static final MessageType MSG_TYPE_KILLED = MessageType.intern("combat.KILLED");
	public static final MessageType MSG_TYPE_PLAYER_KILL_PLAYER = MessageType.intern("combat.PLAYER_KILL_PLAYER");
	public static final MessageType MSG_TYPE_PVP_ATTACK = MessageType.intern("combat.pvpAttack");
	public static final MessageType MSG_TYPE_TARGETS_OUTLAW = MessageType.intern("combat.TARGETS_OUTLAW");
