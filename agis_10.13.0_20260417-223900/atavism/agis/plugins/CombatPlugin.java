...
import atavism.agis.effects.MorphEffect;
...
public class CombatPlugin extends atavism.server.engine.EnginePlugin {
	...
	public void onActivate() {
		...
		//	types.add(CombatClient.MSG_TYPE_TARGET_TYPE);
			...
			mobFilterTypes.add(CombatClient.MSG_TYPE_PVP_ATTACK);
			...
			mobFilterTypes.add(AgisInventoryClient.MSG_TYPE_CURRENCY_ALTERED);
			...
			filter.addType(CombatClient.MSG_TYPE_TARGETS_OUTLAW);
	...
	protected void registerHooks() {
		...
		getHookManager().addHook(CombatClient.MSG_TYPE_TARGETS_OUTLAW, new TargetPvpChaoticHook());
		...
		getHookManager().addHook(CombatClient.MSG_TYPE_PVP_ATTACK, new PvpAttackHook());
		getHookManager().addHook(AgisInventoryClient.MSG_TYPE_CURRENCY_ALTERED, new CurrencyAlteredHook());
	...
	void loadData() {
		...
		String karma = cDB.loadGameSetting("PVP_KARMA_POINTS_PER_KILL");
		if (karma != null) {
			PVP_KARMA_POINTS_PER_KILL = Integer.parseInt(karma);
			log.info("Game Settings PVP_KARMA_POINTS_PER_KILL  set to " + PVP_KARMA_POINTS_PER_KILL);
		}

		String karmaReduce = cDB.loadGameSetting("PVP_KARMA_POINTS_REDUCE");
		if (karma != null) {
			PVP_KARMA_POINTS_REDUCE = Integer.parseInt(karmaReduce);
			log.info("Game Settings PVP_KARMA_POINTS_REDUCE  set to " + PVP_KARMA_POINTS_REDUCE);
		}
		String pvpmarktimeout = cDB.loadGameSetting("PVP_MARK_TIMEOUT");
		if (pvpmarktimeout != null) {
			PVP_MARK_TIMEOUT = Integer.parseInt(pvpmarktimeout);
			log.info("Game Settings PVP_MARK_TIMEOUT  set to " + PVP_MARK_TIMEOUT);
		}
	...
	class PvpAttackHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			CombatInfo info = getCombatInfo(oid);
			if (Log.loggingDebug)
				log.debug("PvpAttackHook: obj: " + oid + " info: " + info+" state="+state);
			info.setProperty("pvpAttack", state == 1);
			return true;
		}
	}

	private class CurrencyAlteredHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			PropertyMessage rmMsg = (PropertyMessage) msg;
			OID oid = rmMsg.getSubject();
			int currency = (int)rmMsg.getProperty("currency");
			long amount = (long)rmMsg.getProperty("amount");
			CombatInfo info = getCombatInfo(oid);

			if(!info.isMob()) {
				AtomicBoolean altered = new AtomicBoolean(false);
				int rank = info.getPvpRank();
				PVP_RANKS.forEach((k, v) -> {
					if (v.getCurrency() == currency) {
						if(v.getValue() > amount) {
							if(rank!= k) {
								altered.set(true);
								info.setPvpRank(k);
								info.setProperty("pvpRankName", v.getTitle());
							}
						}
					}
				});
				if(!altered.get()) {
					WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(oid,oid);
					tpm.setProperty("pvpRankName", info.getProperty("pvpRankName"));
					tpm.setProperty("pvpRank", info.getPvpRank());
					...
					for (String s : params) {
						...
						switch (s) {
							...
							default:
								...
								if(obj.getProperty(s) instanceof AgisStat ) {
									responeParams.put(s, obj.statGetCurrentValue(s));
								}else{
									responeParams.put(s,obj.getProperty(s));
								}
			...
			for (String s : params.keySet()) {
				...
				switch (s) {
					...
					case "setPendingGroupInvite":
						...
						break;
					case "pvpRank":
						int rank = obj.getPvpRank();
						PvpRank pr =PVP_RANKS.getOrDefault(rank,  new PvpRank() );
						if(pr.getEffectId()>0)
							CombatClient.removeEffect(obj.getOid(),pr.getEffectId());
						obj.setPvpRank((int)params.get(s));
						rank = obj.getPvpRank();
						pr = PVP_RANKS.getOrDefault(rank,  new PvpRank() );
						if(pr.getEffectId()>0)
							CombatClient.applyEffect(obj.getOid(),pr.getEffectId());
						break;
					default:
						obj.setProperty(s,params.get(s));
...
*/
	...
	class AbilityVectorHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if(aas == null) {
				if(Log.loggingWarn)log.warn("AbilityVectorHook: got update vector but ability is not running playerOid="+playerOid+" abilityId="+abilityId);
			} else {
	...
	class StartAbilityHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if (ability.isToggle()) {
				...
				if(ability.isPvpEnterExit() && obj.inCombat() && obj.currentRunAbilities().containsKey(ability.getID())){
					ExtendedCombatMessages.sendAbilityFailMessage(obj, AbilityResult.CANT_EXIT_PVP, abilityID,ability.getCostProperty());
					return true;
				}
	...
	public static void handleDeath(CombatInfo obj) {
		...
		if(lastCInfo!=null && !lastCInfo.isMob()) {
			...
			if(!obj.isMob()){
				try {
					CombatClient.sendPlayerWasKilled(oid, last);
				} catch (Exception e) {
					log.exception("sendPlayerWasKilled",e);
				}

			}

		}
		Integer lastHitPvpRank = 0;
		if(obj.isMob() && lastCInfo!=null && !lastCInfo.isMob()) {
			Integer karmaPoints = lastCInfo.getIntProperty("karmaPoints");
			if(karmaPoints!=null && karmaPoints > 0) {
				karmaPoints -= PVP_KARMA_POINTS_REDUCE;
				if(karmaPoints < 0) karmaPoints = 0;
			}else{
				karmaPoints=0;
			}
			if(karmaPoints==0 && lastCInfo.getPvpMark() == 2){
				lastCInfo.setPvpMark(0);
			}
			lastCInfo.setProperty("karmaPoints", karmaPoints);
			EnginePlugin.setObjectPropertyNoResponse(lastCInfo.getOid(), Namespace.FACTION, "karmaPoints", karmaPoints);
//			PropertyMessage	propMsg = new PropertyMessage(lastCInfo.getOid());
//			propMsg.setProperty("karmaPoints",karmaPoints);
//			Engine.getAgent().sendBroadcast(propMsg);
			WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(lastCInfo.getOid(),lastCInfo.getOid());
			tpm.setProperty("karmaPoints",karmaPoints);
			Engine.getAgent().sendBroadcast(tpm);
			lastHitPvpRank =  lastCInfo.getIntProperty(FactionStateInfo.PVP_RANK_PROP);

		}
		if (attackers != null)
			for (OID o : attackers) {
				CombatInfo CInfo = getCombatInfo(o);
				if (!obj.isMob() && CInfo!=null && !CInfo.isMob()) {
					CombatPlugin.addListsRankingData(o, AchievementsClient.PVP_KILL, 1);
				}
			}
		if(!obj.isMob()) {
			int pvpMode = (int) obj.getProperty(FactionStateInfo.PVP_MODE_PROP);
			Integer killedPvpRank = obj.getIntProperty(FactionStateInfo.PVP_RANK_PROP);
			log.debug("handleDeath: pvpMode:"+pvpMode+" killedPvpRank:"+killedPvpRank);
			if(killedPvpRank==null )
				killedPvpRank = PVP_RANKS.keySet().iterator().next();
			log.debug("handleDeath: | pvpMode:"+pvpMode+" pvpRank:"+killedPvpRank);
			PvpRank pr = PVP_RANKS.getOrDefault(killedPvpRank, new PvpRank());
			log.debug("handleDeath: pr:"+pr);
			int pvpMark = obj.getPvpMark();
			int rankdiff =0;
			boolean aply = false;
			if (pvpMode > -1) {
				for (OID o : attackers) {
					CombatInfo aCInfo = getCombatInfo(o);
					if (aCInfo != null && !aCInfo.isMob()) {
						int lpvpMode = (int) aCInfo.getProperty(FactionStateInfo.PVP_MODE_PROP);
						if(lpvpMode == 2 && pvpMode == 2 ){
							int apvpMark = aCInfo.getPvpMark();
							Integer karmaPoints = aCInfo.getIntProperty("karmaPoints");
							if( pvpMark == 0 && (apvpMark == 1 || apvpMark == 2 )) {
								aCInfo.setPvpMark(2);
								if (karmaPoints != null && karmaPoints >= 0) {
									karmaPoints += PVP_KARMA_POINTS_PER_KILL;
									if (karmaPoints < 0) karmaPoints = 0;
								} else if (karmaPoints == null) {
									karmaPoints = PVP_KARMA_POINTS_PER_KILL;
								}
							} else if( pvpMark == 1 && apvpMark == 1 ) {
								obj.setPvpMark(0);
								aCInfo.setPvpMark(0);
							}
							aCInfo.setProperty("karmaPoints", karmaPoints);
							EnginePlugin.setObjectPropertyNoResponse(aCInfo.getOid(), Namespace.FACTION, "karmaPoints", karmaPoints);
//							PropertyMessage	propMsg = new PropertyMessage(aCInfo.getOid());
//							propMsg.setProperty("karmaPoints",karmaPoints);
//							Engine.getAgent().sendBroadcast(propMsg);

							WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(aCInfo.getOid(),aCInfo.getOid());
							tpm.setProperty("karmaPoints",karmaPoints);
							Engine.getAgent().sendBroadcast(tpm);
						}
						Integer lpvpRank =  aCInfo.getIntProperty(FactionStateInfo.PVP_RANK_PROP);
						log.debug("handleDeath: lpvpMode:"+lpvpMode+" lpvpRank:"+lpvpRank);
						if (lpvpMode > -1) {
							aply = true;
							if(lpvpRank==null)
								lpvpRank=PVP_RANKS.keySet().iterator().next();;
							PvpRank lpr = PVP_RANKS.getOrDefault(lpvpRank, new PvpRank());
							rankdiff = killedPvpRank - lpvpRank;
							//-1			2             3
							log.debug("handleDeath: lpr:"+lpr);
							if (lpr.getCurrency() > 0 && lpr.getIncrease() > 0) {
								long value = lpr.getIncrease();
								if(rankdiff < 0) {
									if(lpr.getIncreaseDiffBelow().containsKey(Math.abs(rankdiff))){
										value = lpr.getIncreaseDiffBelow().get(Math.abs(rankdiff));
									} else if(lpr.getMaxKeyIncreaseDiffBelow() > 0){
										value =lpr.getIncreaseDiffBelow().get(lpr.getMaxKeyIncreaseDiffBelow());
									}
								} else if(rankdiff > 0){
									if(lpr.getIncreaseDiffAbove().containsKey(Math.abs(rankdiff))){
										value = lpr.getIncreaseDiffAbove().get(Math.abs(rankdiff));
									} else if(lpr.getMaxKeyIncreaseDiffAbove() > 0){
										value= lpr.getIncreaseDiffAbove().get(lpr.getMaxKeyIncreaseDiffAbove());;
									}
								}
								if(value!=0)
									AgisInventoryClient.alterCurrency(o, lpr.getCurrency(), value);
							}
						}
					}
				}
			...
			if (aply) {
				if (pr.getCurrency() > 0 && pr.getReduce() > 0) {
					rankdiff = killedPvpRank - lastHitPvpRank;
					long value = pr.getReduce();
					if(rankdiff > 0){
						if(pr.getReduceDiffBelow().containsKey(Math.abs(rankdiff))){
							value = pr.getReduceDiffBelow().get(Math.abs(rankdiff));
						} else if(pr.getMaxKeyReduceDiffBelow() > 0){
							value =pr.getReduceDiffBelow().get(pr.getMaxKeyReduceDiffBelow());
						}
					} else if(rankdiff < 0){
						if(pr.getReduceDiffAbove().containsKey(Math.abs(rankdiff))){
							value = pr.getReduceDiffAbove().get(Math.abs(rankdiff));
						} else if(pr.getMaxKeyReduceDiffAbove() > 0){
							value =pr.getReduceDiffAbove().get(pr.getMaxKeyReduceDiffAbove());
						}
					}
					if(value!=0)
						AgisInventoryClient.alterCurrency(oid, pr.getCurrency(), -value);
				}
			}
		}
		//if (Log.loggingTrace)
	...
	class PropertyHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			Serializable pvpMode = propMsg.getProperty(FactionStateInfo.PVP_MODE_PROP);
			if(pvpMode!=null && (int)pvpMode == -1){
				CombatInfo info = getCombatInfo(objOid);
				if(info.currentRunAbilities().size() > 0) {
					HashMap<Integer,AgisAbilityState> list = new HashMap<Integer,AgisAbilityState>(info.currentRunAbilities());
					int i = 1;
					for(AgisAbilityState aas : list.values()) {
						if(aas.getAbility().isPvpEnterExit()) {
								AgisAbility.deactivateAbility(aas);
						}
					}
				}
	...
	class DespawnedHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			if (obj != null)
				obj.setCombatState(false);
			// Force dismount and remove morph effects on despawn/disconnect
			try {
				forceDismountAndRemoveMorphEffects(objOid);
			} catch (Exception e) {
				log.error("DESPAWN: Error during forced dismount/morph removal for player " + objOid + ": " + e.getMessage());
			}
			// Also tell all attackers to stop
			Set<OID> attackers = getAttackers(objOid);
			if (attackers != null) {
	 ...
	 * @param oid
	 */
	private static void forceDismountAndRemoveMorphEffects(OID oid) {
		CombatInfo player = getCombatInfo(oid);
		if (player == null) {
			if (Log.loggingDebug)
				Log.debug("LOGOUT: forceDismountAndRemoveMorphEffects - player with OID:" + oid + " is null");
			return;
		}

		List<EffectState> effectsToRemove = new ArrayList<>();

		// Find mount and morph effects to remove
		for (EffectState state : player.getCurrentEffects()) {
			if (state.getEffect() != null) {
				if (state.getEffect() instanceof MountEffect) {
					effectsToRemove.add(state);
					if (Log.loggingDebug)
						Log.debug("LOGOUT: Found mount effect to remove: " + state.getEffectID());
				} else if (state.getEffect() instanceof MorphEffect) {
					effectsToRemove.add(state);
					if (Log.loggingDebug)
						Log.debug("LOGOUT: Found morph effect to remove: " + state.getEffectID());
				}
			}
		}

		// Remove all found mount and morph effects
		for (EffectState effect : effectsToRemove) {
			try {
				AgisEffect.removeEffect(effect, player);
				if (Log.loggingDebug)
					Log.debug("LOGOUT: Successfully removed effect: " + effect.getEffectID());
			} catch (Exception e) {
				log.error("LOGOUT: Error removing effect " + effect.getEffectID() + " from player " + oid + ": " + e.getMessage());
			}
		}

		if (Log.loggingDebug && effectsToRemove.size() > 0) {
			Log.debug("LOGOUT: Removed " + effectsToRemove.size() + " mount/morph effects from player " + oid);
		}
	...
	private class TargetPvpChaoticHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			PropertyMessage pmsg = (PropertyMessage) msg;
			OID objOid = pmsg.getSubject();
			if(Log.loggingDebug)Log.debug("TargetPvpChaoticHook: Start");
			CombatInfo info = getCombatInfo(objOid);
			if(info!=null) {
				ArrayList<OID> list = (ArrayList<OID>) pmsg.getProperty("targets");

				Set<OID> set = new HashSet<OID>(list);
				info.SetPvpChaoticTargets(set);
			}else{
				if(Log.loggingDebug)Log.debug("TargetPvpChaoticHook: CombatInfor is null for "+objOid);
			}
			if(Log.loggingDebug)Log.debug("TargetPvpChaoticHook: End");
			return true;
		}
	}
...
*/
	...
	class LoginHook implements Hook {
		public boolean processMessage(Message msg, int flags) {
			...
			info.setProperty("pvpAttack", false);
			...
			if (equipList != null) {
				...
				Serializable gs = info.getProperty(GEAR_SCORE);
				if(gs==null && CombatPlugin.statDefMap.containsKey(GEAR_SCORE)) {
					AgisStatDef statDef3 = statDefMap.get(CombatPlugin.GEAR_SCORE);
					info.setProperty(CombatPlugin.GEAR_SCORE, new AgisStat(statDef3.getId(), CombatPlugin.GEAR_SCORE, 0));
				}
				for (String statName : CombatPlugin.statDefMap.keySet()) {
					...
					AgisStat stat = (AgisStat) info.getProperty(statName);
					if (stat == null) {
						log.debug("LoginHook stat " + statName + " is null for player ");
						continue;
					}
			...
			for (EffectState es : currentEffects) {
				if(es.getEffect()!=null && es.getEffect() instanceof StatEffect){
					StatEffect se = (StatEffect) es.getEffect();
					se.activate(es);
				}
			}
			...
			Integer karmaPoints = info.getIntProperty("karmaPoints");
			if(karmaPoints==null) {
				karmaPoints = 0;
				info.setProperty("karmaPoints",karmaPoints);
			}
			EnginePlugin.setObjectPropertyNoResponse(playerOid, Namespace.FACTION, "karmaPoints", karmaPoints);
			if(karmaPoints>0)
				info.setPvpMark(2);
//
//			if(!info.isMob()) {
...
//				});
//				if(!altered) {
//					WorldManagerClient.TargetedPropertyMessage tpm = new WorldManagerClient.TargetedPropertyMessage(playerOid,playerOid);
//					tpm.setProperty("pvpRankName", info.getProperty("pvpRankName"));
//					tpm.setProperty("pvpRank", info.getPvpRank());
//					Engine.getAgent().sendBroadcast(tpm);
//
...
//			}
			...
			AgisStatDef statDef3 = statDefMap.get(CombatPlugin.GEAR_SCORE);
			if(statDef3 != null) {
				statMap.put(CombatPlugin.GEAR_SCORE, new AgisStat(statDef3.getId(), CombatPlugin.GEAR_SCORE, 0));
			}
		}
...
//	public static String MOVEMENT_SPEED_STAT = null;
	...
	public static String GEAR_SCORE = "gearScore";
	...
	public static LinkedHashMap<Integer,PvpRank> PVP_RANKS;
	...
	public static Integer PVP_KARMA_POINTS_PER_KILL =20;
	public static Integer PVP_KARMA_POINTS_REDUCE =2;
	public static long PVP_MARK_TIMEOUT =20;
