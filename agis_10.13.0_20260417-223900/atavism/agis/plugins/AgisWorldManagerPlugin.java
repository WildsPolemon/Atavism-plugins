...
//	 	mobRPCSubId = Engine.getAgent().createSubscription(mobRPCFilter, this, MessageAgent.RESPONDER);
		...
		String pvpMessage = cDB.loadGameSetting("PVP_KILLED_MESSAGE");
		if (pvpMessage != null)
			PVP_KILLED_MESSAGE = pvpMessage;
    ...
    class LoginHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
			...
			players.computeIfAbsent(instanceOid, k -> new HashSet<>()).add(playerOid);
            // Make sure the model is set correctly
			Entity entity = getWorldManagerEntity(playerOid);
			if(entity != null) {
				LinkedList<Integer> completedDialogues = (LinkedList<Integer>) entity.getProperty("completedDialogues");
				if (completedDialogues != null) {
					LinkedList<Integer> completedDialoguesDeduplicated = new LinkedList<>(new LinkedHashSet<>(completedDialogues));
					entity.setProperty("completedDialogues", completedDialoguesDeduplicated);
				}
			}
	...
	ConcurrentHashMap<OID, Set<OID>> players = new ConcurrentHashMap<>();
	/**
    ...
    class LogoutHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            players.forEach((k,v) -> v.remove(playerOid));
	...
	protected boolean IsAnyPlayerInInstance(OID instaceOID){

		if(Log.loggingDebug)
			log.debug("IsAnyPlayerInInstance: "+instaceOID+" "+players);
		if(players.containsKey(instaceOID))
			return !players.get(instaceOID).isEmpty();
		return false;
	}
    	...
    	public boolean processMessage(Message msg, int flags) {
			...
			try {
				...
				if (objInfo.objType == ObjectTypes.player) {
					...
					players.computeIfAbsent(instanceOid, k -> new HashSet<>()).add(objOid);
	...
	class DespawnedHook implements Hook	 {
		public boolean processMessage(Message msg, int flags) {
			...
			players.computeIfAbsent(instanceOid, k -> new HashSet<>()).remove(objOid);
			return true;
		}
	}
...
//            log.error("sendDCMessage: not a agisobj: " + obj);
    ...
    public static String PVP_KILLED_MESSAGE = "{KILLED} was killed by {ATTACKER}";
