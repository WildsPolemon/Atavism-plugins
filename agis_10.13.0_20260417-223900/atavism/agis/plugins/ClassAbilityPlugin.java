		...
		if (attacker.isGrouped()) {
			...
			for (OID groupMemberOid : groupMembersInRange) {
				...
				if (!handledOids.contains(groupMemberOid)) {
					...
					if (attackerLevel - targetLevel <= EXP_MAX_LEVEL_DIFFERENCE) {
						...
						if(!target.isMob() && !groupMember.isMob()) {
							CombatPlugin.addListsRankingData(groupMemberOid, AchievementsClient.PVP_KILL, target.getID());
						}
			...
			if (attackerLevel - targetLevel <= EXP_MAX_LEVEL_DIFFERENCE) {
				...
				if(!target.isMob() && !attacker.isMob()) {
					CombatPlugin.addListsRankingData(attackerOid, AchievementsClient.PVP_KILL, target.getID());
				}
