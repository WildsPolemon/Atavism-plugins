...
public class GuildClient {
	...
	public static void sendLeaveOrEnterGuild(OID oid,int gid, String action) {
		PropertyMessage msg = new PropertyMessage();
		msg.setMsgType(MSG_TYPE_LEAVE_OR_ENTER_GUILD);
		msg.setSubject(oid);
		msg.setProperty("gid", gid);
		msg.setProperty("action", action);
		Engine.getAgent().sendBroadcast(msg);
	}

	/**
	 * Check if can Create guild
	 * @param oid
	 * @return
	 */
	public static boolean canCreateGuild(OID oid){
		PropertyMessage msg = new PropertyMessage(MSG_TYPE_CAN_CREATE_GUILD, oid);
		Log.debug("GUILD CLIENT: canCreateGuild hit 1");
		boolean response = Engine.getAgent().sendRPCReturnBoolean(msg);
		Log.debug("GUILD CLIENT: canCreateGuild response="+response);
		return response;
	}

	public static final MessageType MSG_TYPE_LEAVE_OR_ENTER_GUILD = MessageType.intern("guild.LEAVE_OR_ENTER_GUILD");
	...
	public static final MessageType MSG_TYPE_CAN_CREATE_GUILD = MessageType.intern("faction.CAN_CREATE_GUILD");
