...
public class EquipHelper {
	...
	{	
		...
		if (itemToEquip != null) {
			...
			if (setId > 0) {					
				ItemSetProfile isp = ObjectManagerClient.getItemSetProfile(setId);
				for (ItemSetLevel isl : isp.GetLevels()) {
					if (isl.GetNumberOfParts() == itemSets.getOrDefault(setId,0)) {
						// Apply Effects
						if (isl.GetEffects().size() > 0) {
							for (int i = 0; i < isl.GetEffects().size(); i++) {
	...
	public static void UpdateEquiperStats(OID oid, AgisItem itemToEquip, AgisItem itemToUnequip, CombatInfo obj) {
		if(Log.loggingDebug)Log.debug("EquipHelper.UpdateEquiperStats oid="+oid+" itemToEquip =" + itemToEquip + " and itemToUnequip = " + itemToUnequip);
	...
	// Unequip old item first
		if (itemToUnequip != null) {
			...
			if (itemType.equals("Weapon")) {
				...
				if(Log.loggingDebug)Log.debug("ITEMS: slot for item " + itemToUnequip.getName() + " is: " + slot);
				...
				if(Log.loggingDebug)Log.debug("SPEED: removing attack speed modifier");
			    	...
			    	if (autoAttack != null && autoAttack > 0) {
						...
						if(Log.loggingDebug)Log.debug("UpdateEquiperStats: itemToUnequip:" + itemToUnequip + " autoAttack:" + autoAttack);
    		...
    		for (Integer i : keys) {
    			if (itemSockets.get(i).GetItemOid()!=null) {
					...
					if(Log.loggingDebug)	Log.debug("EQUIP:unequipping item altering stat aistats: " + aistats );
    			...
    			//		HashMap<String, Integer> aistats = (HashMap) ai.getProperty("bonusStats");
    					for (String statName : CombatPlugin.STAT_LIST) {
    						//if (stats.containsKey(statName)) {
							if(Log.loggingDebug)Log.debug("EQUIP:unequipping item altering stat: " + statName );
						...
						if(Log.loggingDebug)Log.debug("EQUIP: unequipping item socketing stats is null OID:"+itemSockets.get(i).GetItemOid());
			...
			setId = itemToUnequip.getIntProperty("item_set");
    		obj.statRemoveModifier(CombatPlugin.GEAR_SCORE, itemToUnequip.getOid(), false);
			
		}

		if(Log.loggingDebug)Log.debug("EQUIP: setId:" + setId);
		if (setId > 0) {
			HashMap<Integer, Integer> itemSets = (HashMap) AgisInventoryClient.getSetsEquiped(obj.getOid());
			if(Log.loggingDebug)Log.debug("EQUIP: itemSets:" + itemSets);
			...
			if(Log.loggingDebug)Log.debug("EQUIP: stats cleared for set:" + setId);

			HashMap<String, Integer> setStats = new HashMap<String, Integer>();
			HashMap<String, Integer> setStatsp = new HashMap<String, Integer>();
			ItemSetProfile isp = ObjectManagerClient.getItemSetProfile(setId);
			if(Log.loggingDebug)Log.debug("EQUIP: setId:" + setId + " ItemSetProfile:" + isp + " levels:" + isp.GetLevels().size());

			for (ItemSetLevel isl : isp.GetLevels()) {
				if(Log.loggingDebug)Log.debug("EQUIP: ItemSetLevel:" + isl);
			...
			for (String statName : setStats.keySet()) {
				...
				if(Log.loggingDebug)Log.debug("EQUIP: Item Set altering stat: " + statName + " by: " + value);
				//if (value > 0) {
					obj.statAddModifier(statName, "ItsemSet" + setId, value, false);
				if(Log.loggingDebug)Log.debug("EQUIP: Item Set altered stat: " + statName + " by: " + value);
				//}
			}
			for (String statName : setStatsp.keySet()) {
				int value = setStatsp.get(statName);
				if(Log.loggingDebug)Log.debug("EQUIP: item Set altering percent stat: " + statName + " by: " + value);
				//if (value > 0) {
					obj.statAddPercentModifier(statName, "ItsemSet" + setId, value, false);
				if(Log.loggingDebug)Log.debug("EQUIP: Item Set altered percent stat: " + statName + " by: " + value);
				//}
			}
			// }
		}
		if (itemToEquip != null) {
			...
			//	String weaponType = "Armed";
				if(Log.loggingDebug)Log.debug("ITEMS: weapon type for item " + itemToEquip.getName() + " is: " + weaponType + " in slot: " + slot);
				...
				if (slot.contains("Off Hand")) {
			    	...
			    	if (autoAttack != null && autoAttack > 0) {
						...
						if(Log.loggingDebug)Log.debug("UpdateEquiperStats: itemToEquip:"+itemToEquip+" autoAttack:"+autoAttack);
			...
			if (itemType.equals("Weapon")) {
				int damage = (Integer) itemToEquip.getProperty("damage");
		     //  	obj.statAddModifier("dmg-base", itemToEquip.getOid(), damage, false); // Damage is added on
				if(Log.loggingDebug)   Log.debug("Equip Item: "+obj.getName()+" dmg-base "+damage +" to equip "+ itemToEquip.getName());
		    	int damageMax = (Integer) itemToEquip.getProperty("damageMax");
		    	if (damageMax < damage) damageMax = damage;
		    //	obj.statAddModifier("dmg-max", itemToEquip.getOid(), damageMax, false); // Damage is added on
			...
			for (String statName: stats.keySet()) {
				...
				if(Log.loggingDebug)Log.debug("EQUIP: item:"+itemToEquip+" equipping stat: " + statName + " by: " + value);
			}
		...
		//	HashMap<String, Integer> stats = (HashMap<String, Integer>) itemToUnequip.getProperty("bonusStats");
			...
			for (String statName: stats.keySet()) {
				int value = stats.get(statName);
				if(Log.loggingDebug)Log.debug("EQUIP: item:"+itemToEquip+" equipping with enchant stat: " + statName + " by: " + value);
			}
			...
			for (String statName: stats.keySet()) {
				int value = stats.get(statName);
				if(Log.loggingDebug)Log.debug("EQUIP: equipping item altering stat: " + statName + " by: " + value);
			...
			if (itemType.equals("Weapon")) {
				...
				if(Log.loggingDebug)Log.debug("SPEED: weapon speed: " + speed + " and player speed: " + obj.statGetBaseValue(CombatPlugin.ATTACK_SPEED_STAT) + " gives mod: " + speed_mod);
		    	obj.statAddModifier(CombatPlugin.ATTACK_SPEED_STAT, itemToEquip.getOid(), speed_mod, false);
			}
			HashMap<Integer, SocketInfo> itemSockets = (HashMap<Integer, SocketInfo>) itemToEquip.getProperty("sockets");
    		Set<Integer> keys = itemSockets.keySet();
			if(Log.loggingDebug)Log.debug("Equip: item:"+itemToEquip+" sokets:"+keys);
    		for (Integer i : keys) {
    			if (itemSockets.get(i).GetItemOid()!=null) {
    			//	AgisItem ai = AgisInventoryPlugin.getAgisItem(itemSockets.get(i).GetItemOid());
    			//	if (ai!=null) {
					...
					if(Log.loggingDebug)Log.debug("EQUIP:equipping item altering stat aistats: " + aistats +" Item in socket "+itemSockets.get(i).GetItemOid());
    				if (aistats!=null) {
    	...
    	setId = itemToEquip.getIntProperty("item_set");
		obj.statAddModifier(CombatPlugin.GEAR_SCORE, itemToEquip.getOid(),gear_score, false);
		...
		if(Log.loggingDebug)Log.debug("Equip Helper itemToEquip="+itemToEquip+" set weaponType="+weaponType);
		EnginePlugin.setObjectProperty(oid, CombatClient.NAMESPACE, "weaponType", weaponType);
		if(Log.loggingDebug)Log.debug("Equip Helper itemToEquip="+itemToEquip+" set toolType="+toolType);
		EnginePlugin.setObjectProperty(oid, CombatClient.NAMESPACE, "toolType", toolType);
		
		int g = obj.statGetCurrentValue(CombatPlugin.GEAR_SCORE);
		...
		if(Log.loggingDebug)Log.debug("EquipHelper.UpdateEquiperStats oid="+oid+" END");
	}
	...
	public static void updateDisplay(OID mobOid, String displayVal, AgisEquipSlot slot, String itemVal) {
		if(Log.loggingDebug)Log.debug("updateDisplay: oid=" + mobOid + " slot=" + slot + " displayVal=" + displayVal + " itemVal=" + itemVal);
