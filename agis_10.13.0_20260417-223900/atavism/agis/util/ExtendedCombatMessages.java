...
public class ExtendedCombatMessages {
	...
	public static void sendAbilityFailMessage(CombatInfo obj, AbilityResult result, int abilityID, String prop) {
        ...
        //	failmessageType = 9;
			...
			failmessageType = 27;
		else if (result == AbilityResult.NO_PVP)
			failmessageType = 28;
		else if (result == AbilityResult.EFFECT_NOT_ON_CASTER)
			failmessageType = 29;
		else if (result == AbilityResult.EFFECT_NOT_ON_TARGET)
			failmessageType = 30;
		else if (result == AbilityResult.EFFECT_ON_CASTER)
			failmessageType = 31;
		else if (result == AbilityResult.EFFECT_ON_TARGET)
			failmessageType = 32;
		else if (result == AbilityResult.CANT_EXIT_PVP)
			failmessageType = 33;
		Log.debug("ABILITY: sending fail message: " + failmessageType+" "+result);
