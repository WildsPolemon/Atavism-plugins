...
public class AgisMob extends AgisObject {
    ...
    public Object clone() throws CloneNotSupportedException {
        lock.lock();
        try {
            AgisMob it =  (AgisMob)super.clone();
            return it;
        } finally {
            lock.unlock();
        }
    ...
    static {
        try {
            BeanInfo info = Introspector.getBeanInfo(AgisMob.class);
            PropertyDescriptor[] propertyDescriptors = info.getPropertyDescriptors();
            for (int i = 0; i < propertyDescriptors.length; ++i) {
                PropertyDescriptor pd = propertyDescriptors[i];
                if (pd.getName().equals("activeAbilities")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("currentAbility")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("dmgTable")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("maxMoveSpeed")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("speedBonus")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("pdBonus")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("endurance")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("constitution")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("equipMap")) {
                    pd.setValue("transient", Boolean.TRUE);
                }else if (pd.getName().equals("skillMap")) {
                    pd.setValue("transient", Boolean.TRUE);
                }
