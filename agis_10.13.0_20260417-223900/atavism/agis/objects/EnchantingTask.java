...
//				socketItem.setProperty("sockets", itemSockets);
	//	
	    ...
	    //	obj.statAddModifier("dmg-max", ai.getOid(), damageMax, false); // Damage is added on
		    ...
		    enchantStats.put(CombatPlugin.GEAR_SCORE, gearScore);
		}
		...
		if (enchantLevel > 0) {
			...
			if (ep.GetLevels().containsKey(enchantLevel)) {
				for (int e = 1; e <= enchantLevel; e++) {
					enchantStats.replace(CombatPlugin.GEAR_SCORE,enchantStats.get(CombatPlugin.GEAR_SCORE)+ep.GetLevels().get(e).GetGearScoreValue()+(int)(enchantStats.get(CombatPlugin.GEAR_SCORE) * ep.GetLevels().get(e).GetGearScoreValuePercentage()));
