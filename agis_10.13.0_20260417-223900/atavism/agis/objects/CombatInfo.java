...
public class CombatInfo extends Entity implements Runnable, Cooldown.CooldownObject {
    ...
    public String toString() {
        return "[CombatInfo: " + getName() + ":" + getOid() +":"+getID()+ "]";
    }
    ...
    public void setAutoAttack(OID newTarget) {
		...
		if(abilityID  < 1) {
			if (Log.loggingDebug)  Log.debug("AUTO: "+getOid()+" ability is not set for auto attack break");
			target = null;
			return;
		}
        lock.lock();
...
//		scheduledExecutioner = 
	...
	private transient int pvp_rank = 0;
	...
	public HashMap<Integer, Collection<Integer>> getAbilitiesRef(){
		return new HashMap<Integer, Collection<Integer>>(abilities);
	}
	...
	public HashMap<Integer, ArrayList<ArrayList<String>>> getActionsRef(){
		return new HashMap<Integer, ArrayList<ArrayList<String>>>(actions);
	}
...
*/
	...
	protected transient Set<OID> pvpChaoticTargets = new HashSet<OID>();

	public void SetPvpChaoticTargets(Set<OID> targets) { this.pvpChaoticTargets = targets; }
	public Set<OID> getPvpChaoticTargets() {return pvpChaoticTargets; }
	// Not currently used
	...
	transient ScheduledFuture<?> pvpMarkTask = null;
	transient PvpMarkTimer pvpMarkTimer=null;
	...
	public AgisAbilityState getRunAbility(int id){
		List<AgisAbilityState> ra = new ArrayList<>(runAbilities);
		for (AgisAbilityState aas :ra ) {
	...
	public void addRunAbilities(AgisAbilityState state) {
		List<AgisAbilityState> ra = new ArrayList<>(runAbilities);
		for (AgisAbilityState aas :ra ) {
			if(aas.getTimeRemaining()< -60_000){
				runAbilities.remove(aas);
			}
		}
	...
	public void setPvpMark(int mark){
		if(pvpMark==0)
			pvpMarkTime = System.currentTimeMillis();
		pvpMark = mark;

		if(pvpMarkTask!=null)
			pvpMarkTask.cancel(true);
		if(pvpMarkTimer == null)
			pvpMarkTimer = new PvpMarkTimer();
		pvpMarkTask = Engine.getExecutor().schedule(pvpMarkTimer, CombatPlugin.PVP_MARK_TIMEOUT, TimeUnit.SECONDS);

//		TargetedPropertyMessage propMsg = new TargetedPropertyMessage(getOwnerOid(),getOwnerOid());
		PropertyMessage propMsg = new PropertyMessage(getOwnerOid());
		propMsg.setProperty("pvpMark", mark);
		propMsg.setProperty("pvpMarkTime", (int)CombatPlugin.PVP_MARK_TIMEOUT);
		Engine.getAgent().sendBroadcast(propMsg);
		if (Log.loggingDebug)Log.debug("setPvpMark: " +getOid()+" "+ mark+" End");
	}
	transient int pvpMark=0;
	transient long pvpMarkTime=0L;
	...
	public class PvpMarkTimer implements Runnable {
		public PvpMarkTimer() {
			if(Log.loggingDebug) Log.debug("PvpMarkTimer: Create");
		}
		...
		public void run() {
			pvpMarkTask=null;
			// Check user still has items
			if(Log.loggingDebug) Log.debug("PvpMarkTimer: running remove invite task for " + getOid());
			if(pvpMark == 1 ) {
				setPvpMark(0);
			}
			if(Log.loggingDebug) 	Log.debug("PvpMarkTimer: END");
	...
	public Object clone() throws CloneNotSupportedException {
		lock.lock();
		try {
			CombatInfo it =  (CombatInfo)super.clone();
			it.setBonuses((HashMap<String, BonusSettings>) getBonuses().clone());
			it.setEffects((HashMap<Integer, HashSet<EffectState>>) getEffects().clone());
			it.setSkillInfos((HashMap<Integer, SkillInfo>) getSkillInfos().clone());
			it.setAbilities((HashMap<Integer, Collection<Integer>>) getAbilitiesRef().clone());
			it.setActions((HashMap<Integer, ArrayList<ArrayList<String>>>) getActionsRef().clone());
			it.setVitalityStats((HashMap<String, Long>) getVitalityStats().clone());
			return it;
		} finally {
			lock.unlock();
		}
		...
		try {
			...
			for (int i = 0; i < propertyDescriptors.length; ++i) {
				...
				if (pd.getName().equals("currentAction")) {
					...
					pd.setValue("transient", Boolean.TRUE);
				} else if (pd.getName().equals("skills")) {
					pd.setValue("transient", Boolean.TRUE);
				} else if (pd.getName().equals("currentRunAbilities")) {
					pd.setValue("transient", Boolean.TRUE);
				} else if (pd.getName().equals("runAbilities")) {
					pd.setValue("transient", Boolean.TRUE);
				} else if (pd.getName().equals("pvpMark")) {
					pd.setValue("transient", Boolean.TRUE);
				} else if (pd.getName().equals("pvpMarkTime")) {
					pd.setValue("transient", Boolean.TRUE);
				}
