...
public class Guild implements Serializable {
	...
	public void handleCommand(OID oid, String commandType, OID targetOid, String commandData) {
		...
		if (commandType.equals("invite")) {
			...
			if (targetOid != null) {
				if(!GuildClient.canCreateGuild(oid)){
					Log.warn("GUILD: player attempted to invite to guild, but faction not allow invite target a guild");
					ChatClient.sendObjChatMsg(oid, 1, "You cannot invite this target to guild");
					return ;
				}
				int rep = FactionClient.getStance(oid, targetOid);
				if(rep < FactionPlugin.Neutral){
					String targetName = aDB.getCharacterNameByOid(targetOid);
					EventMessageHelper.SendErrorEvent(targetOid, EventMessageHelper.CANT_INVITE, 0, targetName);
					return;
				}
...
//		Log.error("GUILD: addRank "+rankName+"|"+rankID+"|"+_rankId+"|"+guildID);
    ...
    public void sendMemberData(GuildMember updatedMember, String action) {
		...
		if((action.equals("Remove")||action.equals("Add") )&& updatedMember.status == MEMBER_STATUS_ONLINE) {
			GuildClient.sendLeaveOrEnterGuild(updatedMember.oid, guildID, action);
