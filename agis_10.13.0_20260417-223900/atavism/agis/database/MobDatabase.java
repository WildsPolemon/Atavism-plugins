            ...
            try (ResultSet rs = queries.executeSelect(ps)) {
                while (rs.next()) {
                    ...
                    tmpl.put(WorldManagerClient.NAMESPACE, WorldManagerClient.TEMPL_PERCEPTION_RADIUS, 75);
						...
						tmpl.put(WorldManagerClient.NAMESPACE, "race",  rs.getInt("race_id"));
					...
					tmpl.put(Namespace.FACTION, FactionStateInfo.PVP_MODE_PROP, -1);
					tmpl.put(Namespace.FACTION, FactionStateInfo.PVP_RANK_PROP, -1);
...
//    }
		    ...
		    try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
						...
						f.setCanEnterGuild(rs.getBoolean("canEnterGuild"));
	...
	public HashMap<String, Serializable> LoadFactionPrefabData() {
		HashMap<String, Serializable> props = new HashMap<String, Serializable>();

		int c = 0;
		try (PreparedStatement ps = queries.prepare("SELECT * FROM factions where category=1 AND isactive = 1")) {
			try (ResultSet rs = queries.executeSelect(ps)) {
				if (rs != null) {
					while (rs.next()) {
						int id = rs.getInt("id");
...
//						props.put("i" + c + "tooltip", tooltip);
			...
			Log.dumpStack("LoadFactionPrefabData: SQLException:" + e);
			e.printStackTrace();
		}
		return props;
	}
				...
				if (rs != null) {
					...
					while (rs.next()) {
						...
						if (!choose) {
							...
							if (itemCount > 0) {
								props.put("i" + c + "rewardCh" + rewardLevel + "_" + itC.get(rewardLevel), item);
								props.put("i" + c + "rewardChc" + rewardLevel + "_" + itC.get(rewardLevel), itemCount);
...
//						String name = HelperFunctions.readEncodedString(rs.getBytes("name"));
	...
	public int writeSpawnData(SpawnData sd, Point loc, Quaternion orient, BehaviorTemplate behavTmpl, int instanceID) {
		...
		try (PreparedStatement stmt = queries.prepare("INSERT INTO " + tableName + " (" + columnNames 
			...
			stmt.setFloat(42,  (((int)behavTmpl.getRoamDelayMin())/1000F));
			stmt.setFloat(43,  (((int)behavTmpl.getRoamDelayMax())/1000F));
	...
	public int editSpawnData(SpawnData sd, int spawnID, Point loc, Quaternion orient, BehaviorTemplate behavTmpl) {
		...
		try (PreparedStatement stmt = queries.prepare("UPDATE " + tableName + " set mobTemplate=?, locX=?, locY=?, locZ=?, orientX=?, orientY=?, "
			...
			stmt.setFloat(38,  (((int)behavTmpl.getRoamDelayMin())/1000F));
			stmt.setFloat(39,  (((int)behavTmpl.getRoamDelayMax())/1000F));
