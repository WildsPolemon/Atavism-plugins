...
//							if(min != 0)
//								baseStatDef.setMin(min);
							if(max != 0)
								baseStatDef.setMax(max);
...
//						int stat_id = rs.getInt("stat_id");
	...
	public LinkedHashMap<Integer,PvpRank> LoadPvpRanks() {
		LinkedHashMap<Integer,PvpRank> list = new LinkedHashMap<>();
		// First load in base stats
		try (PreparedStatement ps = queries.prepare("SELECT * FROM pvp_ranks_settings order by value asc")) {
			try (ResultSet rs = queries.executeSelect(ps)) {
				if (rs != null) {
					while (rs.next()) {
						...
						int id = rs.getInt("id");
						rank.setId(id);
						rank.setTitle( HelperFunctions.readEncodedString(rs.getBytes("title")));
						rank.setDiffAbove( HelperFunctions.readEncodedString(rs.getBytes("diff_above")));
						rank.setDiffBelow( HelperFunctions.readEncodedString(rs.getBytes("diff_below")));
						rank.setValue(rs.getInt("value"));
						rank.setCurrency(rs.getInt("currency_id"));
						rank.setIncrease(rs.getInt("currency_increase"));
						rank.setReduce(rs.getInt("currency_reduce"));
						rank.setEffectId(rs.getInt("effect_id"));
			...
			Log.dumpStack("LoadPvpRanks: SQLException:"+e);
			e.printStackTrace();
		}
	...
	public HashMap<String, Serializable> LoadPvpRanksPrefabData() {
		HashMap<String, Serializable> props = new HashMap<String, Serializable>();

		int c = 0;
		try (PreparedStatement ps = queries.prepare("SELECT * FROM pvp_ranks_settings ")) {
			try (ResultSet rs = queries.executeSelect(ps)) {
				if (rs != null) {
					while (rs.next()) {
						int id = rs.getInt("id");
						...
						props.put("i" + c + "id", id);
						props.put("i" + c + "title", HelperFunctions.readEncodedString(rs.getBytes("title")));
//						String tooltip = HelperFunctions.readEncodedString(rs.getBytes("tooltip"));
//						if (tooltip == null)
//							tooltip = "";
//						props.put("i" + c + "tooltip", tooltip);
						props.put("i" + c + "v", rs.getBoolean("value"));
						props.put("i" + c + "cid", rs.getInt("currency_id"));
						props.put("i" + c + "ci", rs.getInt("currency_increase"));
						props.put("i" + c + "cr", rs.getInt("currency_reduce"));
//						props.put("i" + c + "color", rs.getInt("color"));
//						props.put("i" + c + "color", rs.getInt("color"));
//						props.put("i" + c + "color", rs.getInt("color"));
//						props.put("i" + c + "color", rs.getInt("color"));
//						props.put("i" + c + "color", HelperFunctions.readEncodedString(rs.getBytes("color")));
//						props.put("i" + c + "dfi", rs.getBoolean("display_faction_icon"));
//						props.put("i" + c + "dt", rs.getBoolean("display_title"));
//						props.put("i" + c + "dpri", rs.getBoolean("display_pvp_ranking_icon"));
...
//    							list.add(effect);
							...
							if (effect != null)
								list.add(effect);
						}else if (effectMainType.equals("Faction")) {
							AgisEffect effect = loadFactionEffect(rs);
							if (effect != null)
								list.add(effect);
						}else if (effectMainType.equals("ChangeFaction")) {
							AgisEffect effect = loadFactionEffect(rs);
							if (effect != null)
								list.add(effect);
						}
	...
	/**
	 * Load in the specific data for a Faction Effect.
	 * @param rs
	 * @return
	 */
	public AgisEffect loadFactionEffect(ResultSet rs) {
		if(Log.loggingDebug)Log.debug("loadFactionEffect");

		try {
			String effectType = HelperFunctions.readEncodedString(rs.getBytes("effectType"));
			if (effectType.equals("FactionEffect")) {
				FactionEffect effect = new FactionEffect(rs.getInt("id"), HelperFunctions.readEncodedString(rs.getBytes("name")));
				int faction = rs.getInt("intValue1");
				int rep = rs.getInt("intValue2");
				boolean changeDefaultFaction = rs.getBoolean("boolValue1");
				boolean restoreFaction = rs.getBoolean("boolValue2");
				if(Log.loggingDebug)Log.debug("loadFactionEffect faction="+faction+" rep="+rep);
				effect.setFaction(faction);
				effect.setRepValue(rep);
				effect.setChangeDefaultFaction(changeDefaultFaction);
				effect.setRestoreFaction(restoreFaction);
				loadTagsForEffect(effect,rs);
				return effect;
			}else if (effectType.equals("ChangeFactionEffect")) {
				ChangeFactionEffect effect = new ChangeFactionEffect(rs.getInt("id"), HelperFunctions.readEncodedString(rs.getBytes("name")));
				int faction = rs.getInt("intValue1");
				if(Log.loggingDebug)Log.debug("loadFactionEffect faction="+faction);
				effect.setFaction(faction);
				loadTagsForEffect(effect,rs);
				return effect;
			}
		} catch (SQLException e) {
			Log.dumpStack("loadFactionEffect: SQLException:" + e);
			e.printStackTrace();
		}
		if(Log.loggingDebug)Log.debug("loadFactionEffect END");
...
//						props.put("i" + c + "mc",rs.getFloat("miss_chance"));
	...
	public void setAbilityData(ResultSet rs, AgisAbility ability) {
		try {
			...
			ability.setPvpEnterExit(rs.getBoolean("pvp_enter"));
			...
			ability.setCasterEffectConsumed(rs.getBoolean("casterEffectConsumed"));
			ability.setTargetEffectConsumed(rs.getBoolean("targetEffectConsumed"));
			ability.setPulseCasterEffectConsumed(rs.getBoolean("pulseCasterEffectConsumed"));
			ability.setPulseTargetEffectConsumed(rs.getBoolean("pulseTargetEffectConsumed"));
...
//				Log.debug("ABILITY: targetType for ability " + ability.getName() + " to AoE Friendly");
			...
			ability.setUseEnterCombatState(rs.getBoolean("enterCombatState"));
