					...
					if (!ability.isToggle()) {
						...
						if (acd != null) {
							...
							if (subResult == AbilityResult.SUCCESS) {
								float time = acd.getTime()+Math.round(Math.round(ability.getActivationTime() * castingMod))/1000f;
								source.addComboTime(acd.getAbilityId(), time, ability.getID());
								ExtendedCombatMessages.sendCombo(source.getOwnerOid(), ability.getID(), acd.getAbilityId(), time, false, acd.getShowInCenterUi(), acd.getReplaceInSlot());
							} else {
						...
						if(ability.isPvpEnterExit()){
							if(!source.getBooleanProperty(FactionStateInfo.IN_PVP_PROP)) {
								HashMap<String, Serializable> props = new HashMap<String, Serializable>();
								props.put("pvpState", true);
								WorldManagerClient.ExtensionMessage eMsg = new WorldManagerClient.ExtensionMessage(FactionClient.MSG_TYPE_UPDATE_PVP_STATE, getSourceOid(), props);
...
//					}
				...
				case FAILED:
					...
					List<CoordinatedEffectState> cess = new ArrayList<>(getCoordinatedEffectStates());
					for (CoordinatedEffectState ceState : cess) {
						if (ceState != null)
			...
			Log.exception("AgisAbility.updateState ConcurrentModificationException " ,e );
	 ...
	 * @return
	 */
	public List<CoordinatedEffectState> getCoordinatedEffectStates() {
	 ...
	 */
	public void setCoordinatedEffectStates(List<CoordinatedEffectState> coordinatedEffectStates) {
	 ...
	 *
	 */
	transient protected List<CoordinatedEffectState> coordinatedEffectStates = Collections.synchronizedList(new ArrayList<CoordinatedEffectState>());
	...
	public ArrayList<AgisAbilityState> getAbilityStates() {
		return abilityStates;
	}

	public ArrayList<EffectState> getEffectStates() {
		return effectStates;
	}
