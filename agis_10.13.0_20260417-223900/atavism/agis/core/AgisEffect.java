...
//	public boolean resetTime = false;
    ...
    */
    ...
    public static void removeEffect(AgisEffect.EffectState state, CombatInfo target) {
        ...
        Log.debug("EFFECT: removing effect: " + state.getEffectName()+":"+state.effectID);
        try {
            ObjectLockManager.lockAll(requiredLocks);
	...
	public static class EffectState implements Runnable, Serializable, Cloneable {
		public EffectState() {
			defaultName = null;
			defaultValue = null;
		...
		public HashMap<String, Serializable> getParamsRef() { return new HashMap<String, Serializable>(params); }
...
//			lock.lock();
			try {
				EffectState it =  (EffectState)super.clone();
				it.setParams((Map) getParamsRef().clone());
				return it;
			} finally {
//				lock.unlock();
			}
